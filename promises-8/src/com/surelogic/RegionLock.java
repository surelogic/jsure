 ...
 * <h3>Semantics:</h3>
 * 
 * The currently executing thread must be holding the lock denoted by the
 * {@code lockExpression} when the state specified by the {@code regionName} is
 * read or written, unless
 * <ul>
 ...
 * the access occurs within a constructor of the class that declares that state
 * or one of its subclasses, and the object created by the constructor is only
 * accessed by the thread that invoked the constructor.
 * </ul>
 *
 * <p>
 * The locking policy stated by a {@link RegionLock} annotation is likely
 * associated with a higher-level invariant. This annotation does not, however,
 * elicit the higher-level invariant from the programmer.
 * 
 * <h3>Examples:</h3>
 * 
 ...
 * </pre>
 * 
 * The same lock object may be declared to be multiple locks (including to be a
 * {@link PolicyLock}):
 * 
 * <pre>
 * &#064;Regions({
