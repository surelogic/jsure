...
/**
 * A class representing a nonstandard store as defined in deferred nullification
 * paper. A store is represented by a record of five things:
 * <ol>
 * <li>stack depth;
 * <li>set of objects
 * <li>set of pairs (local,object);
 * <li>set of triples (object,unique-field,object). </ul> A local is a source
 * local or a stack position. A source local is represented by the IRNode for
 * the VariableDeclarator node. A stack position is represented by an instance
 * of Integer. An object is a set of the following:
 * <ul>
...
public class Store extends RecordLattice {
  ...
  public static final PseudoVariable undefinedVariable = new PseudoVariable(
      "undefined", STATE_UNDEFINED);

  public static final PseudoVariable borrowedVariable = new PseudoVariable(
      "borrowed", STATE_BORROWED);

  public static final PseudoVariable sharedVariable = new PseudoVariable(
      "shared", STATE_SHARED);
  ...
  public Store(IRNode[] locals) {
    super(new Lattice[] { FlatLattice.prototype, new UnionLattice(),
        new UnionLattice() });
    this.locals = locals;
  ...
  Store errorStore(String error) {
    ...
    not_quite_bottom = not_quite_bottom
        .setStackSize(FlatLattice.newInteger(-1));
  ...
  public Lattice meet(Lattice other) {
    ...
    if (!m.isValid() && !m.equals(top())) {
      ...
      if (!isValid() && !equals(top()))
        return this;
      if (!((Store) other).isValid() && !other.equals(top()))
        return other;
  ...
  public boolean isValid() {
    return getStackSize().inDomain() && !getObjects().isInfinite()
  ...
  public Store pop() {
    ...
    return this.apply(
        new StoreRemove((UnionLattice) nodeSet.addElement(getStackTop())))
        .setStackSize(getStackSize(n - 1));
  }

  /**
   * return an integer indicating the status of the local:
   * <ul>
   * <li>STATE_NULL - null
   * <li>STATE_UNIQUE - unique
   * <li>STATE_SHARED - shared
   * <li>STATE_BORROWED - borrowed
   * <li>STATE_UNDEFINED - undefined
   * </ul>
   * 
   * @see #isUnique
   * @see #isStoreable
  ...
  /**
   * Return an integer indicating the status of an object description (a set):
   * <ul>
   * <li>STATE_UNIQUE - unique
   * <li>STATE_SHARED - shared
   * <li>STATE_BORROWED - borrowed
  ...
  /**
   * Return whether top of statck is defined. (It should also be so.)
   */
  ...
  /*
   * Stack Machine Operations: NB: remember top and bottom are switched from
   * paper to agree with CFA forms. "top" in the paper is "bottom" here. Instead
   * of using a lifted tuple for the store, we instead raise exceptions when
   * something is illegal.
   */

  /**
   * Get an initial state. We do not need to initialize field store because we
   * implicitly add edges (n,field,{}) for all n.
   */
  public Store opStart() {
    ...
    objects = (UnionLattice) objects.addElement(empty).addElement(
        empty.addElement(undefinedVariable)).addElement(
        empty.addElement(borrowedVariable)).addElement(
        empty.addElement(sharedVariable).addElement(borrowedVariable));
  ...
  public Store opStop() {
    if (!isValid())
      return this;
    UnionLattice localSet = (UnionLattice) nodeSet
        .union(new ImmutableHashOrderSet<Object>(locals));
  ...
  public Store opGet(Object local) {
    ...
    if (isDefined(local)) {
      ...
      String name = (local instanceof IRNode) ? DebugUnparser
  ...
  public Store opDup(int fromTop) {
    if (!isValid())
      return this;
    Integer i = (Integer) getStackSize(getStackTop().intValue() - fromTop)
        .getValue();
    return opGet(i);
  }
  ...
  public Store opSet(Object local) {
    if (!isValid())
      return this;
    UnionLattice lset = (UnionLattice) nodeSet.addElement(local);
    return this.apply(new StoreRemove(lset)).apply(
        new StoreAdd(getStackTop(), lset)).pop();
  }
    ...
    if (UniquenessRules.isUnique(fieldDecl)) {
      ...
      try {
        ...
        while (enm.hasMoreElements()) {
          ...
          if (object.contains(n) && objects.contains(object)
      ...
      Store temp = this.apply(new StoreRemove(affected)).apply(
          new StoreAdd(undefinedVariable, affected)).opNew();
      UnionLattice fieldStore = temp.getFieldStore();
      final UnionLattice uniqueNode = (UnionLattice) nodeSet.addElement(temp
          .getStackTop());
      // System.out.println(temp);
      try {
        Iterator it = new EnumerationIterator(temp.getObjects().elements());
        fieldStore = (UnionLattice) fieldStore.addElements(new FilterIterator(
            it) {
          @Override
      ...
      return this.opRelease().opExisting(sharedVariable);
    }
  }
  ...
  public Store opLoadReachable() {
    ...
    if (nodeStatus(affected) != STATE_UNIQUE)
      return errorStore("loaded compromised field");
    return this.apply(new StoreRemove(affected)).apply(
        new StoreAdd(undefinedVariable, affected)).opRelease();
  }
  ...
  public Store opNew() {
    ...
    try {
      ...
      fieldStore = (UnionLattice) fieldStore
          .addElements(new FilterIterator(it) {
            @Override
            protected Object select(Object o) {
              Triple t = (Triple) o;
      ...
      /*
       * for (Enumeration enm=fieldStore.elements(); enm.hasMoreElements();) {
       * Triple t = (Triple)enm.nextElement(); if (t.first().equals(nodeSet)) {
       * fieldStore = (UnionLattice)fieldStore .addElement(new
       * Triple(nset,t.second(),t.third())); } }
       */
    } catch (SetException ex) {
      return this;
    }
  ...
  /**
   * Evaluate a psuedo-variable onto the top of the stack. A psuedo-variable can
  ...
  public Store opRelease() {
    if (!isValid())
      return this;
    UnionLattice nset = (UnionLattice) nodeSet.addElement(getStackTop());
  ...
  public Store opCompromiseNoRelease() {
    ...
    return apply(new StoreAdd(n, (UnionLattice) nodeSet
  ...
  public Store opUndefine() {
    ...
    return apply(
        new StoreAdd(n, (UnionLattice) nodeSet.addElement(undefinedVariable)))
        .opRelease();
  }
  ...
  public Store opEqual(boolean areEqual) {
    ...
    Integer n = getStackTop();
    Integer undertop = getUnderTop();
    return this.filter(new StoreEqual(n, undertop, areEqual)).opRelease()
        .opRelease();
  }
  ...
  protected Store apply(final StoreApply c) {
    ...
    try {
       ...
       * UnionLattice node = (UnionLattice) enum.nextElement(); newObjects =
       * (UnionLattice) newObjects.addElement(c.apply(node)); }
       */
    ...
    try {
       ...
       * { Triple t = (Triple) enum.nextElement(); newFieldStore =
       * (UnionLattice) newFieldStore.addElement( new Triple(
       * c.apply((UnionLattice) t.first()), t.second(), c.apply((UnionLattice)
       * t.third()))); }
       */
      ...
      final Iterator it = new SimpleRemovelessIterator() {
        ...
        protected Object computeNext() {
          if (enm.hasMoreElements()) {
            Triple t = (Triple) enm.nextElement();
            return new Triple(c.apply((UnionLattice) t.first()), t.second(), c
                .apply((UnionLattice) t.third()));
          }
          return noElement;
        }
      };
  ...
  protected Store filter(final StoreFilter f) {
    ...
    try {
      ...
      newObjects = (UnionLattice) newObjects
          .addElements(new FilterIterator(it) {
            @Override
            protected Object select(Object o) {
       ...
       * UnionLattice node = (UnionLattice)enm.nextElement(); if
       * (f.filter(node)) newObjects =
       * (UnionLattice)newObjects.addElement(node); }
       */
    } catch (SetException ex) {
      return this;
    }
    ...
    try {
      // Changed to avoid set ops by not adding one at a time
      Iterator it = new EnumerationIterator(fieldStore.elements());
      newFieldStore = (UnionLattice) newFieldStore
          .addElements(new FilterIterator(it) {
            @Override
            protected Object select(Object o) {
              Triple t = (Triple) o;
      ...
      /*
       * for (Enumeration enm=fieldStore.elements(); enm.hasMoreElements();) {
       * Triple t = (Triple)enm.nextElement(); if
       * (f.filter((UnionLattice)t.first()) &&
       * f.filter((UnionLattice)t.third())) { newFieldStore =
       * (UnionLattice)newFieldStore.addElement(t); } }
       */
    } catch (SetException ex) {
      return this;
    }
  ...
  static final ImmutableHashOrderSet pseudos = new ImmutableHashOrderSet(
      new Object[] { undefinedVariable, borrowedVariable, sharedVariable });
  ...
  /**
   * Check if the field node (or actually, the VariableDeclarator node inside of
   * a FieldDeclaration) is a static field. (This method probably should be put
   * somewhere more generic.)
   * 
   * @param node
   *          VariableDeclarator node inside of a FieldDeclaration
   * @return true if declared static
   */
  public static boolean isStaticField(IRNode node) {
    return JavaNode.getModifier(JJNode.tree.getParent(JJNode.tree
        .getParent(node)), JavaNode.STATIC);
  }
  ...
  public String toString() {
    ...
    if (ss.equals(ss.bottom()))
      return ss.toString();
    StringBuilder sb = new StringBuilder();
    sb.append("Stack depth: " + getStackSize());
    sb.append('\n');
    sb.append("Objects:\n");
    try {
...
class StoreAdd implements StoreApply {
  ...
  public UnionLattice apply(UnionLattice other) {
    if (other.contains(var))
      return (UnionLattice) other.union(additional);
    return other;
  }
}
...
class TestStore {
  ...
  IRNode param = ParameterDeclaration.createNode(Annotations
      .createNode(noNodes), 0, null, "n");
  {
