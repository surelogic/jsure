...
public class ResultsViewContentProvider {
	private static final Logger LOG = SLLogger
			.getLoggerFor(ResultsViewContentProvider.class);
	...
	private void categorizeRecursive(Content node, boolean atRoot,
		...
		for (Map.Entry<Category, Content> entry : categoryToFolder.entrySet()) {
			...
			if (proofDrops.isEmpty() && !infoDrops.isEmpty()) {
				...
				categoryFolder
						.setBaseImageName(atRoot ? CommonImages.IMG_TALLYHO
								: (localConsistent ? CommonImages.IMG_FOLDER
	...
	private ResultsViewContentProvider buildModelOfDropSea_internal() {
		...
		for (ResultDrop id : resultDrops) {
			...
			if (id.isValid()
					&& ((id.getChecks().isEmpty() && id.getTrusts().isEmpty()))) {
