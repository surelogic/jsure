...
public final class TreeNodeResultsViewLabelProvider extends LabelProvider implements
  ...
  public String getText(final Object obj) {
    if (obj instanceof ResultsTreeNode) {
      ...
      if (status == Status.ORIGINAL || status == Status.CHANGED || status == Status.ADDED || status == Status.DELETED) {
  ...
  public Image getImage(final Object obj) {
    if (obj instanceof ResultsTreeNode) {
      ...
      if (!m_showInferences) {
        flags = flags & ~CoE_Constants.INFO_WARNING & ~CoE_Constants.INFO;
      }
      final String name = n.baseImage;
      final ImageDescriptor id =
        AbstractDoubleCheckerView.getMatchingDescriptor(name); 
      final ResultsImageDescriptor rid =
        new ResultsImageDescriptor(id, flags, AbstractDoubleCheckerView.ICONSIZE);
