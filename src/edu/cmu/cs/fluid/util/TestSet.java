/* $Header: /cvs/fluid/fluid/src/edu/cmu/cs/fluid/util/TestSet.java,v 1.15 2007/07/05 18:15:14 aarong Exp $ */
...
class TestSet {
  ...
  public void test(ImmutableHashOrderSet empty, ImmutableHashOrderSet univ) {
  ...
  public void test(ImmutableHashOrderSet empty, ImmutableHashOrderSet univ, Object o1, Object o2,
			  Object o3, Object o4, Object o5)
  {
    ...
    try {
      Iterator e = s34215.iterator();
      while (e.hasNext()) {
        countdown = countdown.removeElement(e.next());
      }
      if (!countdown.equals(empty))
        reportError("" + countdown + " should be empty.");
    } catch (Exception e) {
      reportError("enumeration failed with exception.");
    }

    try {
      i34215.elements();
      reportError("infinite set can be enumerated.");
    } catch (SetException e) {
    }

    Object[] a132153 = new Object[] { o1, o3, o2, o1, o5, o3 };
    List l132153 = Arrays.asList(a132153);
    ImmutableHashOrderSet s1235 = s123.addElement(o5);
    ImmutableHashOrderSet i4 = univ.removeElement(o4);

    ImmutableHashOrderSet testae1 = empty.addElements(l132153.iterator());
    ImmutableHashOrderSet testae2 = i345.addElements(l132153.iterator());

    if (!testae1.equals(s1235)) {
      reportError("addElements gets wrong result for finite: "
			 + testae1);
    }
    if (!testae2.equals(i4)) {
      reportError("addElements gets wrong result for infinite: "
			 + testae2);
    }

    /* now test that constructor uses uniq: */
    testsetrels(s1235, new ImmutableHashOrderSet(a132153),
		true, true, true, true);
    
  }

  void testsetcontains(ImmutableHashOrderSet s, Object o1, Object o2, Object o3,
  ...
  void testsetrels(ImmutableHashOrderSet s1, ImmutableHashOrderSet s2, boolean equal, boolean supset,
			  boolean subset, boolean overlap) {
    if (s1.equals(s2) != equal) {
      reportError(s1 + (equal?" != ":" = ") + s2);
    }
    if (s1.includes(s2) != supset) {
      reportError(s1 + (supset?" !>= ":" >= ") + s2);
    }
    if (s2.includes(s1) != subset) {
      reportError(s1 + (subset?" !<= ":" <= ") + s2);
    }
    if (s2.overlaps(s1) != overlap) {
      reportError(s1 + (overlap?" disj ":" not disj ") + s2);
    }
  }

  void testsetops(ImmutableHashOrderSet s1, ImmutableHashOrderSet s2, ImmutableHashOrderSet union, ImmutableHashOrderSet intersection,
  ...
  void testsetopresult(String op, ImmutableHashOrderSet s1, ImmutableHashOrderSet s2,
			      ImmutableHashOrderSet result, ImmutableHashOrderSet expected) {
    if (!result.equals(expected)) {
      reportError(s1 + " " + op + " " + s2 + " = " +
			 result + " != " + expected);
    }
  }
