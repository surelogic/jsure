...
public abstract class AbstractMessages {  
  ...
  protected static void collectConstantNames(final Class<?> msgClass, final Map<String,String> value2field) {
	  for(Field f : msgClass.getFields()) {
		  if ((f.getModifiers() & Modifier.STATIC) != 0 && f.getType().equals(String.class)) {
			  try {
				  ...
				  value2field.put(value, f.getName());
				  /*
				  if (f.getName().contains("_dsc")) {
					  continue;
				  }
				  System.out.println("  <findingType>");
				  System.out.println("    <id>"+f.getName()+"</id>");
				  System.out.println("    <artifact tool=\"JSure\" mnemonic=\""+f.getName()+"\"/>");
				  System.out.println("    <shortMessage>TBD</shortMessage>");
				  System.out.println("    <info>TBD</info>");
				  System.out.println("    <name>TBD</name>");
				  System.out.println("  </findingType>");
                  */
	  ...
	  /*
	  for(Map.Entry<String, String> e : value2field.entrySet()) {
		  if (e.getValue().contains("_dsc")) {
			  continue;
		  }
