...
class TestBase64 {
  ...
  void test(String[] args) throws IOException {
    ...
    if (args.length == 1) {
      if (args[0].equals("--encode")) {
        ...
        reportError("Error: do not understand flag: " + args[0]);
      }
    }
  }

  void test_read(String test) throws IOException {
  ...
  void test_skip_read(int s, String test) throws IOException {
  ...
  void check_stream(InputStream is, String test) throws IOException {
    byte[] bytes = test.getBytes();
    ...
    for (int i = 0; i < bytes.length; ++i) {
      ...
      if (by < 0) {
        // System.out.println();
        reportError("Error: input ended too soon");
        return;
      } else {
        newbytes[i] = (byte) by;
