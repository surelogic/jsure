...
//  private static final Set<String> namesOfMthSummsEverBuilt = new HashSet<String>();
...
//  public boolean containsGrantOrRevoke = false;
  ...
  //    ColorReqSummaryDrop res = ColorPromises.getReqSummDrop(node);
  //    
  //    if (res == null) {
  //      res = new ColorReqSummaryDrop(proto, Inherited.NO);
  //      ColorPromises.setReqSummDrop(node, res);
  //    }
  //    
  //    return res;
  //  }
  ...
  public static ColorReqSummaryDrop getSummaryFor(IRNode node) {
    ...
    res = ColorPromises.getReqSummDrop(node);
...
//      if ("windowClosing".equalsIgnoreCase(mthName)) {
//        final String longName = JavaNames.genQualifiedMethodConstructorName(node);
//        LOG.fine("found it" + longName);
//      }

      final String msg = "Color Requirements for " + mthFullName;
      
//      if (mthName.equalsIgnoreCase("createVFSSession")) {
//        LOG.fine("found createVFSSession");
//      }
      
//      if (mthFullName.startsWith("jpl.gds.monitor.gui.EvrComposite.createControls")) {
//	LOG.fine("found createControls");
//      }

      LOG.fine("computing " + msg);

      res = new ColorReqSummaryDrop(node);
      res.setNodeAndCompilationUnitDependency(node);
//      namesOfMthSummsEverBuilt.add(mthFullName);
      ColorPromises.setReqSummDrop(node, res);
      
      protos = ColorPromises.getReqDrops(node);
      ...
      //      Collection iProtos = ColorPromises.getInheritedRequireDrops(node);
...
//          ColorRequireDrop proto = protoIter.next();
        for (ColorRequireDrop proto : protos) {
          LOG.fine("adding " + proto.getRenamedExpr() + " to " + msg);
...
//    resDrop = null;
    ...
    if (!ColorPromises.isColorRelevant(node)) {
    ...
    assert (ColorPromises.getReqSummDrop(node) == null);
    ColorPromises.setReqSummDrop(node, this);
