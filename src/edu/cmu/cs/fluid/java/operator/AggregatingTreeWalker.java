 ...
 */
public abstract class AggregatingTreeWalker<ResultType>
{
   ...
   */
  public final Set<ResultType> walkTree( final IRNode root )
  {
    final Set<ResultType> result = new HashSet<ResultType>();
   ...
   * @param root The node to be analyzed
   */
  protected final void walkTree( final Set<ResultType> result, final IRNode root )
  {
    ...
    if( rootOp == Arguments.prototype ) {
      final Iterator<IRNode> args = Arguments.getArgIterator( root );
    ...
    // Skipping ArithUnopExpression because it has subclasses
      ...
      final Iterator<IRNode> elements = 
        ArrayInitializer.getInitIterator( root );
      this.arrayInitializer( result, root, elements );
    } else if( rootOp == ArrayLength.prototype ) {
      this.arrayLength( result, root, ArrayLength.getObject( root ));
    ...
    // Skipping ArrayType, not sure if it is useful
      ...
      final IRNode lhs = AssignExpression.getOp1( root );
      final IRNode rhs = AssignExpression.getOp2( root );
    ...
    // Skipping BinopExpression because it has subclasses
      ...
      final Iterator<IRNode> stmts = BlockStatement.getStmtIterator( root );
    ...
    // Skipping ByteType, not sure if it is useful
      ...
      final Iterator<IRNode> clauses = CatchClauses.getCatchClauseIterator( root );
    ...
    // Skipping CharType, not sure if it is useful
      ...
      final Iterator<IRNode> decls = ClassBody.getDeclIterator( root );
    ...
    // Skipping ConstructionObject because it has subclasses
    else if( ConstructorCall.prototype.includes(rootOp) ) {
      ...
      final Object name = ConstructorDeclaration.getId( root );
    ...
    // Skipping Declaration because it has subclasse
      ...
      final Iterator<IRNode> dims = DimExprs.getSizeIterator( root );
    ...
    else if (rootOp == ElseClause.prototype) {
      final IRNode elze = ElseClause.getElseStmt(root);
      this.elseClause(result, root, elze);
    }
    // Skipping DoubleType
    ...
    // Skipping Expression because it has subclasses
      ...
      final Iterator<IRNode> classes =
        Extensions.getSuperInterfaceIterator( root );
    ...
    */
      ...
      final IRNode elsePart = IfStatement.getElsePart(root);
      this.ifStatement( result, root, cond, thenPart, elsePart );
    } else if( rootOp == Implements.prototype ) {
      final Iterator<IRNode> interfaces = Implements.getIntfIterator( root );
      ...
      final Iterator<IRNode> imports = ImportDeclarations.getImpsIterator( root );
    ...
    /*
    else if( rootOp == PolymorphicMethodCall.prototype ) {
      
    }
    */
    else if( MethodCall.prototype.includes(rootOp) ) {
      MethodCall call     = (MethodCall) rootOp;
      final IRNode args   = call.get_Args( root );
      final String method = MethodCall.getMethod( root );
      final IRNode obj    = call.get_Object( root );
    ...
    // Skipping NamedType
      ...
      this.nestedInterfaceDeclaration( result, root, body, extensions,
					      id, mods ); 
    } else if( NewExpression.prototype.includes(rootOp) ) {
      NewExpression newE = (NewExpression) rootOp;
      final IRNode args = newE.get_Args( root );
      final IRNode type = newE.get_Type( root );
    ...
    } else if (rootOp == NoElseClause.prototype) {
      this.noElseCaluse(result, root);
    ...
    } else if( rootOp == OmittedMethodBody.prototype ) {
      this.omittedMethodBody( result, root );
    }
    // SKipping NumericType
    else if( rootOp == OpAssignExpression.prototype ) {
      final IRNode lhs = OpAssignExpression.getOp1( root );
      final IRNode rhs = OpAssignExpression.getOp2( root );
    ...
    else if (rootOp == OuterObjectSpecifier.prototype) {
      this.outerObjectSpecifier(result, root, 
          OuterObjectSpecifier.getObject(root),
          OuterObjectSpecifier.getCall(root));
    }
    ...
    // Skipping PackageDeclaration
      ...
      this.parameterDeclaration( result, root, id, mods, type );
    } else if ( rootOp == Parameters.prototype ) {
      final Iterator<IRNode> params = Parameters.getFormalIterator( root );
      this.parameters( result, root, params );
    } else if ( rootOp == ParenExpression.prototype ) {
      this.parenExpression( result, root, ParenExpression.getOp( root ) );
    ...
    // Skipping PrimType
      ...
      final IRNode alloc = OuterObjectSpecifier.getCall( root );
      final IRNode type = OuterObjectSpecifier.getObject( root );
    ...
    // Skipping StatementExpression
      ...
      final Iterator<IRNode> stmts = StatementExpressionList.getExprIterator( root );
    ...
    // SuybExpression instance of BinopExpression
      ...
      final Iterator<IRNode> elements = SwitchBlock.getElementIterator( root );
    ...
    // Skipping SwitchLabel
      ...
      final Iterator<IRNode> stmts = SwitchStatements.getStmtsIterator( root );
      ...
      final Iterator<IRNode> types = Throws.getTypeIterator( root );
    ...
    // Skipping TypeDeclaration
      ...
      final Iterator<IRNode> types = TypeDeclarations.getTypesIterator( root );
      this.typeDeclarations( result, root, types );
    } else if (rootOp == TypeDeclarationStatement.prototype) {
      this.typeDeclarationStatement(result, root, 
          TypeDeclarationStatement.getTypedec(root));
    ...
    // UnsignedRightShiftExpression instanceof BinopExpression
    else if( rootOp == VariableUseExpression.prototype ) {
      final Object id = VariableUseExpression.getId( root );
      ...
      final Iterator<IRNode> vars = VariableDeclarators.getVarIterator( root );
  ...
  protected abstract void outerObjectSpecifier(Set<ResultType> result, IRNode root, IRNode object, IRNode call);
  
  protected abstract void noElseCaluse(Set<ResultType> result, IRNode root);

  protected abstract void typeDeclarationStatement(Set<ResultType> result, IRNode root, IRNode typedec);
  ...
  protected abstract void binOpExpression( 
    Set<ResultType> result, Operator op, IRNode root, IRNode op1, IRNode op2 );

  /** List of method call actual arguments
    * @param root The Node of the arguments
    * @param args Iterator of the nodes of the arguments
    */
  protected abstract void arguments( Set<ResultType> result, IRNode root, Iterator<IRNode> args );
  ...
  /** An array allocation.
    ...
    * @param unalloc ?
    */
  protected abstract void arrayCreationExpression( Set<ResultType> result, IRNode root, IRNode alloc,
  ...
  /** An array declaration.
    ...
    * @param dims ?
    */
  protected abstract void arrayDeclaration( Set<ResultType> result, IRNode root, IRNode base,
					   int dims );

  /** An array initializer.
    * @param root The node of the initializer expression
    * @param elements An Iterator of the nodes of element expressions
    */
  protected abstract void arrayInitializer( Set<ResultType> result, IRNode root, Iterator<IRNode> elements );

  protected abstract void arrayLength( Set<ResultType> result, IRNode root, IRNode array );
  ...
  /** Indexing of an array.
    ...
    */
  protected abstract void arrayRefExpression( Set<ResultType> result, IRNode root, IRNode array,
					     IRNode index );
  ...
  protected abstract void assertStatement(
    Set<ResultType> result, IRNode root, IRNode assertion );
  ...
  protected abstract void assertMessageStatement(
    Set<ResultType> result, IRNode root, IRNode assertion, IRNode message );
  ...
  /** An assignment expression.
    ...
    */
  protected abstract void assignExpression( Set<ResultType> result, IRNode root,
					   IRNode lhs, IRNode rhs );

  /** A block of statements.
    * @param root The node of the block
    * @param stmts Iterator of the nodes of the statements in the block
    */
  protected abstract void blockStatement( Set<ResultType> result, IRNode root, Iterator<IRNode> stmts );

  /** An unlabeled break statement.
    * @param root The node of the break statement
    */
  protected abstract void breakStatement( Set<ResultType> result, IRNode root );
  ...
  /** A type case expression.
    ...
    */
  protected abstract void castExpression( Set<ResultType> result, IRNode root,
  ...
  /** A Catch clause in a try statement.
    ...
    */
  protected abstract void catchClause( Set<ResultType> result, IRNode root, IRNode body, IRNode param );

  /** The list of catch clauses of a try statement
    * @param root The node of the list
    * @param clauses Iterator of the nodes of the catch clauses
    */
  protected abstract void catchClauses( Set<ResultType> result, IRNode root, Iterator<IRNode> clauses );

  /** Character constant
    * @param root The node of the expression
    * @param token The character 
    */
  protected abstract void charLiteral( Set<ResultType> result, IRNode root, Object token );

  /** Body of class declaration
    * @param root The node of the body
    * @param decls Iterator of the nodes of the field and method
    * declarations
    */
  protected abstract void classBody( Set<ResultType> result, IRNode root, Iterator<IRNode> decls );
  ...
  /** A class declaration
    ...
    */
  protected abstract void classDeclaration( Set<ResultType> result, IRNode root, IRNode body,
  ...
  /** Use of the special <tt>class</tt> field of an object.
    ...
    */
  protected abstract void classExpression( Set<ResultType> result, IRNode root, IRNode type );
  ...
  /** The <tt>static</tt> class initializer block.
    ...
    */
  protected abstract void classInitializer( Set<ResultType> result, IRNode root, IRNode block,
					   int mods );
  ...
  /** The source file.
    ...
    */
  protected abstract void compilationUnit( Set<ResultType> result, IRNode root, IRNode decls,
   ...
   */
  protected abstract void compiledMethodBody( Set<ResultType> result, IRNode root, Object code);

  /** Binary complement expression
    * @param root The node of the expression
    * @param expr The node of the operand expression
    */
  protected abstract void complementExpression( Set<ResultType> result, IRNode root, IRNode expr );
  ...
  /** Conditional expression
    ...
    */
  protected abstract void conditionalExpression( Set<ResultType> result, IRNode root, IRNode cond,
						IRNode ifTrue,
						IRNode ifFalse );

  /** ??? */
  protected abstract void constantLabel( Set<ResultType> result, IRNode root, IRNode op );

  /** Constructor call.
    * @param root Node of the expression
    * @param object Name of the constructor
    * @param args Iterator of the nodes of the actual parameters
    */
  protected abstract void constructorCall( Set<ResultType> result, IRNode root, IRNode object,
					  IRNode args );
  ...
  /** Declaration of a constructor.
    ...
    */
  protected abstract void constructorDeclaration( Set<ResultType> result, IRNode root, IRNode block,
						 IRNode exceptions,
  ...
  /** An unlabed continue statement.
    ...
    */
  protected abstract void continueStatement( Set<ResultType> result, IRNode root );
  ...
  /** A field/local var declaration statement.
    ...
    */
  protected abstract void declStatement( Set<ResultType> result, IRNode root, int mods, IRNode type,
					IRNode vars );

  /** Use of a default label in a case statement.
    * @param root Node of the label
    */
  protected abstract void defaultLabel( Set<ResultType> result, IRNode root );

  /** ? */
  protected abstract void demandName( Set<ResultType> result, IRNode root, String pkg );

  /** ? */
  protected abstract void dimExprs( Set<ResultType> result, IRNode root, Iterator<IRNode> dims );
  ...
  /** A do-while statement.
    ...
    */
  protected abstract void doStatement( Set<ResultType> result, IRNode root, IRNode cond, IRNode loop );

  protected abstract void elseClause(Set<ResultType> result, IRNode root, IRNode elze);
  
  /** An empty statment.
    * @param root Node of the empty statement
    */
  protected abstract void emptyStatement( Set<ResultType> result, IRNode root );

  /** Expression used as a statement.
    * @param root Node of the statement
    * @param expr Node of the expression
    */
  protected abstract void exprStatement( Set<ResultType> result, IRNode root, IRNode expr );

  /** List of classes/interfaces being extended.
    * @param root Node of the list
    * @param classes Iterator of the classes
    */
  protected abstract void extensions( Set<ResultType> result, IRNode root, Iterator<IRNode> classes );

  /** <tt>false</tt> constant.
    * @param root Node of the expression
    */
  protected abstract void falseExpression( Set<ResultType> result, IRNode root );
  ...
  /** Declaration of a field
    ...
    */
  protected abstract void fieldDeclaration( Set<ResultType> result, IRNode root, int mods, IRNode type,
					   IRNode vars );
  ...
  /** Use of a field
    ...
    */
  protected abstract void fieldRef( Set<ResultType> result, IRNode root, IRNode object, Object id );

  /** Finally clause of a try statement.
    * @param root Node of the clause
    * @param body Node of the body of the clause
    */
  protected abstract void finallyClause( Set<ResultType> result, IRNode root, IRNode body );

  /** Use of a floating point constant.
    * @param root Node of the expression
    * @param token value of the constant
    */
  protected abstract void floatLiteral( Set<ResultType> result, IRNode root, Object token );
  ...
  /** For statement.
    ...
    */
  protected abstract void forStatement( Set<ResultType> result, IRNode root, IRNode init, IRNode cond,
  ...
  /** If-then statement.
    ...
    */
  protected abstract void ifStatement( Set<ResultType> result, IRNode root, IRNode cond,
				      IRNode thenPart, IRNode elsePart );

  /** List of interfaces
    * @param root Node of the list
    * @param interfaces Iterator of the interfaces implemented by a class
    */
  protected abstract void implementsList( Set<ResultType> result, IRNode root, Iterator<IRNode> interfaces );

  /** An import declaration
    * @param root Node of the declaration
    * @param item Node of the package name
    */
  protected abstract void importDeclaration( Set<ResultType> result, IRNode root, IRNode item );

  /** List of import statements
    * @param root Node of the list
    * @param imports Iterator of the nodes of the import statements
    */
  protected abstract void importDeclarations( Set<ResultType> result, IRNode root,
					     Iterator<IRNode> imports );
  ...
  /** Initializer of a non-array variable
    ...
    */
  protected abstract void initialization( Set<ResultType> result, IRNode root, IRNode value );
  ...
  /** Instance of expression
    ...
    */
  protected abstract void instanceOfExpression( Set<ResultType> result, IRNode root, IRNode val,
					       IRNode type );
  ...
  /** Use of an integer constant
    ...
    * @param token Value of the constant
    */
  protected abstract void intLiteral( Set<ResultType> result, IRNode root, Object token );

  /** An interface declaration
    * @param root The node of the interface declaration
    * @param body The node of the interface body
    * @param extensions The node of the list of any super interfaces
    * @param id Name of the interface
    * @param mods Flags for declaration modifers
    */
  protected abstract void interfaceDeclaration( Set<ResultType> result, IRNode root, IRNode body,
  ...
  /** Break with a labeled target
    ...
    */
  protected abstract void labeledBreakStatement( Set<ResultType> result, IRNode root, Object id );

  /** Continue with a labeled target
    * @param root Node of the continue
    * @param id Name of the label
    */
  protected abstract void labeledContinueStatement( Set<ResultType> result, IRNode root, Object id );
  ...
  /** A statement with a label
    ...
    */
  protected abstract void labeledStatement( Set<ResultType> result, IRNode root, Object label,
					   IRNode stmt );
  ...
  /** Body of a method
    ...
    */
  protected abstract void methodBody( Set<ResultType> result, IRNode root, IRNode block );
  ...
  /** Method call with a specific instance
    ...
    */
  protected abstract void methodCall( Set<ResultType> result, IRNode root, IRNode args, Object method,
				     IRNode obj );
  ...
  /** Declaration of a method
    ...
    */
  protected abstract void methodDeclaration( Set<ResultType> result, IRNode root, IRNode body,
  ...
  /** Unary minus expression
    ...
    */
  protected abstract void minusExpression( Set<ResultType> result, IRNode root, IRNode op );

  /** ? */
  protected abstract void name( Set<ResultType> result, IRNode root, String names );

  /** Declartion of a nested class
    * @param root Node of the declaration
    * @param body Node of the body of the declaration
    * @param extension Node of the list of any extended classes
    * @param id Name of the interface
    * @param impls Node of the list of any implemented interfaces
    * @param mods Flags for declaration modifiers
    */
  protected abstract void nestedClassDeclaration( Set<ResultType> result, IRNode root, IRNode body,
						 IRNode extension, Object id,
						 IRNode impls, int mods );
  ...
  /** Declartion of a nested interface
    * @param root Node of the declaration
    * @param body Node of the body of the declaration
    * @param extensions Node of the list of any extended interfaces
    * @param id Name of the interface
    * @param mods Flags for declaration modifiers
    */
  protected abstract void nestedInterfaceDeclaration( Set<ResultType> result, IRNode root, IRNode body,
						     IRNode extensions,
						     Object id, int mods ); 
  ...
  /** Creation of a new instance
    ...
    */
  protected abstract void newExpression( Set<ResultType> result, IRNode root, IRNode args,
					IRNode type );
  ...
  /** Creation of a new instance
    ...
    * @param type Node of the constructor name
    */
  protected abstract void anonClassExpression( Set<ResultType> result, IRNode root, IRNode args,
				   	      IRNode body, IRNode type );

  /** Node for an non-existant array initialization
    * @param root Node of the absent array initialization
    */
  protected abstract void noArrayInitializer( Set<ResultType> result, IRNode root );

  /** Node for an empty class body
    * @param root Node of empty body
    */
  protected abstract void noClassBody( Set<ResultType> result, IRNode root );

  /** Node for a non-existant finally clause
    * @param root Node of empty finally
    */
  protected abstract void noFinally( Set<ResultType> result, IRNode root );

  /** Node for an non-existant initialization
    * @param root Node of the absent initialization
    */
  protected abstract void noInitialization( Set<ResultType> result, IRNode root );

  /** Body of an abstract method
    * @param root Node of the body
    */
  protected abstract void noMethodBody( Set<ResultType> result, IRNode root );
  
  /** Boolean not expression
    * @param root Node of the expression
    * @param op Node of the operand
    */
  protected abstract void notExpression( Set<ResultType> result, IRNode root, IRNode op );

  /** Use of <tt>null</tt>
    * @param root Node of the expression
    */
  protected abstract void nullLiteral( Set<ResultType> result, IRNode root );
  
  /** Body of an abstract method
   * @param root Node of the body
   */
 protected abstract void omittedMethodBody( Set<ResultType> result, IRNode root );
  ...
  /** Operator assignment expression
    ...
    */
  protected abstract void opAssignExpression( Set<ResultType> result, IRNode root, IRNode lhs, 
  ...
  /** Formal arameter declaration
    ...
    */
  protected abstract void parameterDeclaration( Set<ResultType> result, IRNode root, 
					       Object id, int mods,
					       IRNode type );

  /** List of formal paramers to a constructor or method call
    * @param root Node of the list
    * @param params Iterator of the nodes of the parameter declarations
    */
  protected abstract void parameters( Set<ResultType> result, IRNode root, Iterator<IRNode> params );
	 ...
	 * @param node
	 */
	protected void parenExpression(Set<ResultType> result, IRNode root, IRNode node) {
  ...
  /** Unary plus
    ...
    */
  protected abstract void plusExpression( Set<ResultType> result, IRNode root, IRNode expr );

  /** x-- expression
    * @param root Node of the decrement expression
    * @param expr Node of the expression being decremented
    */
  protected abstract void postDecrementExpression( Set<ResultType> result, IRNode root, IRNode expr );

  /** x++ expression
    * @param root Node of the increment expression
    * @param expr Node of the expression being incremented
    */
  protected abstract void postIncrementExpression( Set<ResultType> result, IRNode root, IRNode expr );

  /** --x expression
    * @param root Node of the decrement expression
    * @param expr Node of the expression being decremented
    */
  protected abstract void preDecrementExpression( Set<ResultType> result, IRNode root, IRNode expr );

  /** ++x expression
    * @param root Node of the increment expression
    * @param expr Node of the expression being incremented
    */
  protected abstract void preIncrementExpression( Set<ResultType> result, IRNode root, IRNode expr );

  protected abstract void qualifiedAllocationExpression( Set<ResultType> result, IRNode root, IRNode alloc, IRNode type );
  protected abstract void qualifiedThisExpression( Set<ResultType> result, IRNode root, IRNode type );

  /** Return statement
    * @param root Node of the statement
    * @param val Node of the expression being returned
    */
  protected abstract void returnStatement( Set<ResultType> result, IRNode root, IRNode val );

  /** Comma seperated list of expressions.
    * @param root Node of the list
    * @param stmts Iterator of nodes of the statements
    */
  protected abstract void statementExpressionList( Set<ResultType> result, IRNode root, Iterator<IRNode> stmts );

  /** Use of a string literal.
    * @param root Node of the string
    * @param token The body of the string
    */
  protected abstract void stringLiteral( Set<ResultType> result, IRNode root, Object token );

  /** Use of the "super" expression.
    * @param root The node of the expression
    */
  protected abstract void superExpression( Set<ResultType> result, IRNode root );
 
  /** List of "case" statements.
    * @param root The node of the list
    * @param elements Iterator of the "case" clauses
    */
  protected abstract void switchBlock( Set<ResultType> result, IRNode root, Iterator<IRNode> elements );
  ...
  /** A "case" or "default" clause of a switch statement.
    ...
    */
  protected abstract void switchElement( Set<ResultType> result, IRNode root, IRNode label,
					IRNode stmts );
  ...
  /** A switch statement.
    ...
    */
  protected abstract void switchStatement( Set<ResultType> result, IRNode root, IRNode block,
					  IRNode expr );

  /** Statements following a "case" of "default" label
    * @param root The node of the statements
    * @param stmts Iterator of the statements
    */
  protected abstract void switchStatements( Set<ResultType> result, IRNode root, Iterator<IRNode> stmts );
  ...
  /** Synchronized statement.
    ...
    */
  protected abstract void synchronizedStatement( Set<ResultType> result, IRNode root, IRNode block,
						IRNode lock );

  /** Use of the object receiver
    * @param root The node of the use
    */
  protected abstract void thisExpression( Set<ResultType> result, IRNode root );

  /** A throw statement
    * @param root The root of the throw statement
    * @param val The node of the expression being thrown
    */
  protected abstract void throwStatement( Set<ResultType> result, IRNode root, IRNode val );

  /** List of thrown exceptions
    * @param root The node of the list
    * @param types Iterator of the exceptions
    */
  protected abstract void throwsList( Set<ResultType> result, IRNode root, Iterator<IRNode> types );

  /** Use of the <tt>true</tt> constant.
    * @param root The node of the <tt>true</tt>
    */
  protected abstract void trueExpression( Set<ResultType> result, IRNode root );
  ...
  /** A try statement.
    ...
    */
  protected abstract void tryStatement( Set<ResultType> result, IRNode root, IRNode block,
				       IRNode catchPart, IRNode finallyPart );

  protected abstract void typeDeclarations( Set<ResultType> result, IRNode root, Iterator<IRNode> types );

  /** A type expression
    * @param root The node of the type expression
    * @param type The type of the expression
    */
  protected abstract void typeExpression( Set<ResultType> result, IRNode root, IRNode type );

  /** An implicit package declaration
    * @param root The node of the implicit declaration
    */
  protected abstract void unnamedPackageDeclaration( Set<ResultType> result, IRNode root );

  /** Use of a variable.
    * @param root The node of the use
    * @param id 
    */
  protected abstract void useExpression( Set<ResultType> result, IRNode root, Object id );
  ...
  /** Declaration of a variable, including initializer
    ...
    */
  protected abstract void variableDeclarator( Set<ResultType> result, IRNode root, Object id,
					     int dims, IRNode init );
  
  /** List of variables in a declaration.
    * @param root The node of the list
    * @param vars Iterator of the nodes of the variable names
    */
  protected abstract void variableDeclarators( Set<ResultType> result, IRNode root, Iterator<IRNode> vars );
  
  /** Empty return statement.
    * @param root The node of the return statement
    */
  protected abstract void voidReturnStatement( Set<ResultType> result, IRNode root );
  ...
  /** While statement.
    ...
    */
  protected abstract void whileStatement( Set<ResultType> result, IRNode root, IRNode cond,
					 IRNode loop ); 
}
