...
//        ModuleModel.setModuleInformationIsConsistent(false);
    ...
    private void checkTypePlacement(IRNode node) {
      if (!currMod.isLeafModule()) {
        ...
        final Collection<ModuleDrop> promiseSet =  ModuleDrop.findModuleDrop(node);
        for (ModuleDrop modPromise : promiseSet) {
          if (modPromise != null) {
...
//    ModuleModel.purgeUnusedModuleModels();
    ...
    ModuleDrop.buildModuleDropResults();
    VisibilityDrop.visibilityPrePost();
    ModuleDrop.moduleDropPrePost();
  }
