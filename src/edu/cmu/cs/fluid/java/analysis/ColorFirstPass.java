...
public final class ColorFirstPass {
  ...
  public static void preBuild() {
    ColorPromises.colorDropsEnabled = true;
  }
 ...
 //    final String cuName = JavaNames.getFullTypeName(cu);
      ...
      if (ColorPromises.areColorImportsProcessed(cu)) {
        return;
      }
...
//      final String name = JavaNames.genMethodConstructorName(node);
      ...
      try {
        if (!ColorPromises.areColorImportsProcessed(theCUsRoot)) {
          ColorPromises.setColorImportsProcessed(theCUsRoot, true);
    ...
    private void processColorImports(final IRNode node, 
      ...
      if (ColorPromises.areColorImportsProcessed(node)) return;
//      Collection<ColorImportDrop> imports = ColorPromises.getColorImports(node);
      ...
      for (ColorImportDrop ciDrop : imports) {
        ...
        Collection<ColorRenameDrop> someImportedRenames = 
          ColorRenameDrop.getRenamesHere(ciDrop.getImportedUnit());
...
//      if (className.startsWith("imptest")) {
//        LOG.fine("found imptest");
//      }
      ...
      ColorPromises.setColorImportsProcessed(nd, true);
...
//          final IRNode obj = FieldRef.getObject(fieldRef);
  ...
  class ColorRenameWalkerNew extends ColorStructVisitor {
    ...
    public void doRenameReqDrops(IRNode node) {
      ...
      Collection<ColorRequireDrop> reqDrops = ColorPromises.getReqDrops(node);
    ...
    private void doLibraryMthRenameWalk(IRNode node) {
      if (!ColorRenameDrop.globalHaveRenames() || 
          ColorPromises.areColorRenamesProcessed(node)) {
        return;
      }
      ...
      try {
        ...
        ColorPromises.setColorRenamesProcessed(node, true);
      } finally {
        ColorRenamePerCU.endACU(saveCookie);
      }
    }
...
//            doLibraryRefImportWalk(fieldRef);
    ...
    private void doLibraryRefRename(final IRNode fieldRef) {
      ...
      if (ColorPromises.areColorRenamesProcessed(encClass)) {
	return;
      }
...
//      final ColorStaticClass hisClass = ColorStaticClass.getStaticClass(encClass);
	...
	Set<RegionColorDeclDrop> rcdDrops = 
	  ColorPromises.getMutableRegionColorDeclsSet(encClass);
	...
	ColorPromises.setColorRenamesProcessed(encClass, true);
      }  finally {
	ColorRenamePerCU.endACU(saveCookie);
      }
...
//        }
        Set<RegionColorDeclDrop> rcdDrops = 
          ColorPromises.getMutableRegionColorDeclsSet(nd);
        for (RegionColorDeclDrop anRCD : rcdDrops) {
          CExpr newExpr = renameACExpr(anRCD.getUserConstraint(), anRCD);
          anRCD.setRenamedConstraint(newExpr);
        }
        
        ColorPromises.setColorRenamesProcessed(nd, true);
...
//        }
    ...
    private Void handleBlockLikeOps(IRNode node) {
      Set<ColorGrantDrop> grants = ColorPromises.getMutableColorGrantSet(node);
      Set<ColorRevokeDrop> revokes = ColorPromises.getMutableColorRevokeSet(node);
...
//          currStruct.addChild(theRef);
    ...
    public Void visitNamedPackageDeclaration(IRNode node) {
      currCUStruct.colorIncompatibles =
        ColorPromises.getColorIncompatibles(node);
...
//      }
//
//      ColorPromises.setColorImportsProcessed(node, true);
//      
...
//    public Void visitBlockStatement(IRNode node) {
//
//      Collection<ColorGrantDrop> grants = ColorPromises.getColorGrants(node);
//      if (grants != null) {
//        // process the grants
...
//      }
//
//      Collection<ColorRevokeDrop> revokes = ColorPromises.getColorRevokes(node);
//      if (revokes != null) {
...
//        
//        Set<RegionColorDeclDrop> rcdDrops = 
//          ColorPromises.getMutableRegionColorDeclsSet(node);
//        for (RegionColorDeclDrop anRCD : rcdDrops) {
//          CExpr newExpr = renameACExpr(anRCD.getUserConstraint(), anRCD);
//          anRCD.setRenamedConstraint(newExpr);
//        }
...
//      // ColorIncompatible must be processed now.
//
//      Collection<ColorIncompatibleDrop> colorIncs = ColorPromises
...
//      final boolean saveColorsNeedBodyTraversal = colorsNeedBodyTraversal;
      ...
      try {
        Collection<ColorIncompatibleDrop> colorIncs = 
          ColorPromises.getColorIncompatibles(node.getNode());
        for (ColorIncompatibleDrop inc : colorIncs) {
...
//    public Void visitBlockStatement(IRNode node) {
//
//      if (!ColorPromises.getColorGrants(node).isEmpty()
//          || !ColorPromises.getColorRevokes(node).isEmpty()) {
//        colorsNeedBodyTraversal = true;
//      }
//
