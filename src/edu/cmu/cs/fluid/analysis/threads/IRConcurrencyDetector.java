  ...
  private class FastVisitor extends Visitor<Object> {
    ...
    public Object visitNewExpression(IRNode n) {
      ...
      if (isThreadSubtype(type)) {
        reportInference(threadCreationCategory, JavaNames.getTypeName(decl)
            + " instance created", n);
      } else if (implementsRunnable(type)) {
        reportInference(runnableCreationCategory, JavaNames.getTypeName(decl)
            + " instance created", n);
      }
      return null;
    }
    ...
    public Object visitMethodCall(IRNode n) {
      ...
      if (name.equals("start")) {
        ...
        IJavaType type = JavaTypeFactory.convertNodeTypeToIJavaType(t, analysisContext.binder);
        if (type instanceof IJavaDeclaredType && isThreadStart(m, (IJavaDeclaredType)type)) {
          reportInference(threadStartsCategory, JavaNames.getTypeName(t)
              + " started", n);
        }
      }
      return null;
