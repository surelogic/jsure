 ...
 * <code>")</code>
 * <p>
 * Where <i>promise</i> is assumed on all declarations matched by <i>target</i>.
 * For example, the below assumes a <code>&#064;Starts("nothing")</code>
 * annotation on the no-argument constructor for
 * {@link IllegalArgumentException}. The assumption supports the
 * <code>&#064;Starts("nothing")</code> promised on the constructor
 ...
 * Within the scope of the compilation unit in which the {@link Assume}
 * annotation appears, from the point of view of the verifying analyses, it is
 * as if these virtual promises were annotated by the programmer. If a virtual
 * promise exists as a real promise then the assumption has no semantics. If the
 * virtual promise does not exist as a real promise then it is "trusted" (i.e.,
 * not verified). The results reported from tools, such as SureLogic JSure,
 * should make it clear to the programmer that the promise has not been
 * verified.
 * <p>
