...
public abstract class AbstractIRAnalysis<T extends IBinderClient, Q extends IAnalysisGranule> extends ConcurrentAnalysis<Q>
    implements IIRAnalysis<Q> {
  // private IIRProject project;
  String label;
  IBinder binder;
  protected final ThreadLocalAnalyses analyses = new ThreadLocalAnalyses();

  protected AbstractIRAnalysis(boolean inParallel) {
    super(inParallel);
  }
  ...
  /*
   * public void preAnalysis(IIRAnalysisEnvironment env) { // Nothing to do }
   */
  ...
  public final void analyzeBegin(IIRAnalysisEnvironment env, final IIRProject p) {
    ...
    /*
     * if (!runInParallel() || singleThreaded) { setupAnalysis(old != p); } else
     * { runInParallel(Void.class, nulls, new Procedure<Void>() { public void
     * op(Void v) { //System.out.println(old+" ?= "+p); setupAnalysis(old != p);
     * } }); }
     */
   ...
   * //System.out.println(Thread.currentThread()+" : "+flushAnalysis()+", "
   * +diffProject); if (flushAnalysis() || diffProject || analysis.get() ==
   * null) { runInVersion(new edu.cmu.cs.fluid.util.AbstractRunner() { public
   * void run() { analysis.remove(); T a = constructIRAnalysis(binder);
   * analysis.set(a); } }); } }
   */
