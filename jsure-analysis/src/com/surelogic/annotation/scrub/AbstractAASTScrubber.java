...
public abstract class AbstractAASTScrubber<A extends IAASTRootNode, P extends PromiseDrop<? super A>>
   ...
   */
  public AbstractAASTScrubber(String name, Class<A> c, IPromiseDropStorage<P> stor, ScrubberType type, String[] before,
  ...
  public AbstractAASTScrubber(String name, Class<A> c, IPromiseDropStorage<P> stor, ScrubberType type, String... deps) {
    this(name, c, stor, type, NONE, ScrubberOrder.NORMAL, deps);
  }

  public AbstractAASTScrubber(String name, Class<A> c, IPromiseDropStorage<P> stor, ScrubberType type, ScrubberOrder order,
      String... deps) {
   ...
   */
  public AbstractAASTScrubber(String name, Class<A> c, IPromiseDropStorage<P> stor) {
    this(name, c, stor, ScrubberType.UNORDERED, NONE, ScrubberOrder.NORMAL, NONE);
  }

  public AbstractAASTScrubber(ISingleAnnotationParseRule<A, P> rule, ScrubberType type, String... deps) {
    this(rule, type, NONE, ScrubberOrder.NORMAL, deps);
  }

  public AbstractAASTScrubber(ISingleAnnotationParseRule<A, P> rule, ScrubberType type, String[] before, String... deps) {
    this(rule, type, before, ScrubberOrder.NORMAL, deps);
  }

  public AbstractAASTScrubber(ISingleAnnotationParseRule<A, P> rule, ScrubberType type, String[] before, ScrubberOrder order,
      String... deps) {
    this(rule.name(), rule.getAASTType(), rule.getStorage(), type, before, order, deps);
  }
  ...
  protected final boolean checkForTypeBinding(AASTNode node, boolean rv) {
    if (node instanceof ResolvableToType) {
      ...
      if (!r.typeExists() && getContext() != null) {
        getContext().reportError("Couldn't resolve a type for " + node + " on " + current, node);
        rv = false;
      }
    }
    return rv;
  ...
  protected void processAASTsForType(IAnnotationTraversalCallback<A> cb, IRNode decl, List<A> l) {
    if (StorageType.SEQ.equals(stor.type())) {
       ...
       * 
       * // Sort to process in a consistent order Collections.sort(l,
       * aastComparator);
       */
      ...
      Multimap<IRNode, A> annos = ArrayListMultimap.create();
      ;
      for (A a : l) {
        annos.put(a.getPromisedFor(), a);
      }
      final List<Map.Entry<IRNode, Collection<A>>> nodes = new ArrayList<>(annos.asMap().entrySet());
      ...
      /*
       * Unordered among the nodes for(Map.Entry<IRNode,Collection<A>> e :
       * annos.entrySet()) { processAASTsByNode(e.getValue()); }
       */
    }
  }
  ...
  /**
   * Written for boolean/node promises that can really only take one AAST per
   * node, checking for conflicts
   * 
   * @param cb
   */
          ...
          if (success) {
            for (A a2 : l) {
              if (a2 != a) {
                ...
                /*
                 * if ("Starts nothing".equals(a.toString())) { PromiseDrop<?>
                 * sp2 = AASTStore.getPromiseSource(a2); if (sp != null) {
                 * System.out.println("Got Starts nothing: "
                 * +DebugUnparser.toString(sp.getNode())); } if (sp2 != null) {
                 * System.out.println("Got Starts nothing: "
                 * +DebugUnparser.toString(sp2.getNode())); } }
                 */
                if (sp != null) {
                  getContext().reportError(sp.getMessage() + " (" + JavaNames.getFullName(sp.getNode())
                      + ") overridden by explicit annotation " + a + " (" + JavaNames.getFullName(a.getPromisedFor()) + ")",
                      sp.getAAST());
                } else {
                  getContext().reportError(a2 + " (" + JavaNames.getFullName(a2.getPromisedFor())
                      + ") overridden by explicit annotation " + a + " (" + JavaNames.getFullName(a.getPromisedFor()) + ")", a2);
                }
   ...
   */
  protected PromiseDrop<? super A> makePromiseDrop(IAnnotationTraversalCallback<A> cb, A ast, boolean isAssumption) {
