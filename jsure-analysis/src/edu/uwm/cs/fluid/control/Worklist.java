...
public interface Worklist extends Cloneable {
  ...
  /**
   * Get the worklist ready to start adding new nodes and do a new analysis. At
   * least one call to initialize <em>must</em> occur before each call to
   * {@link #start()}. After this call and before the call to {@link #start()},
   * there may be one or more calls to {@link #add(ControlNode)}.
   */
   ...
   * {@link #initialize(ControlNode)} (with following calls to
   * {@link #add(ControlNode)}. This method may do a lot of prepatory work, but
   * should be no more than O(n lg n) where n is the size of the CFG.
   */
  public void start();
  ...
  /**
   * Indicate that the given node must be in the worklist. After this happens,
   * the worklist must guarantee that this node is returned at some point in the
   * future from {@link #next()}.
   * 
   * @param node
   *          node to add to worklist
   */
