...
public class ForwardAnalysis<T, L extends Lattice<T>, XFER extends ForwardTransfer<T>> extends FlowAnalysis<T, L> {
  ...
  static final Logger LOG = SLLogger.getLogger("FLUID.analysis");
  ...
  final LabeledLattice.UnaryOp<T, IRNode> conditionalTrueTransfer = new LabeledLattice.UnaryOp<T, IRNode>() {
    @Override
  ...
  final LabeledLattice.UnaryOp<T, IRNode> conditionalFalseTransfer = new LabeledLattice.UnaryOp<T, IRNode>() {
    @Override
    public T operate(T x, IRNode arg) {
  ...
  final LabeledLattice.UnaryOp<T, ComponentChoice> componentChoiceTrueTransfer = new LabeledLattice.UnaryOp<T, ComponentChoice>() {
    @Override
  ...
  final LabeledLattice.UnaryOp<T, SubcomponentChoice> subcomponentChoiceTrueTransfer = new LabeledLattice.UnaryOp<T, SubcomponentChoice>() {
    @Override
  ...
  final LabeledLattice.UnaryOp<T, ComponentChoice> componentChoiceFalseTransfer = new LabeledLattice.UnaryOp<T, ComponentChoice>() {
    @Override
    public T operate(T x, ComponentChoice arg) {
  ...
  final LabeledLattice.UnaryOp<T, SubcomponentChoice> subcomponentChoiceFalseTransfer = new LabeledLattice.UnaryOp<T, SubcomponentChoice>() {
    @Override
    public T operate(T x, SubcomponentChoice arg) {
  ...
  final LabeledLattice.LabelOp<LabelTest> testLabelTrueOp = new LabeledLattice.LabelOp<LabelTest>() {
    @Override
  ...
  final LabeledLattice.LabelOp<LabelTest> testLabelFalseOp = new LabeledLattice.LabelOp<LabelTest>() {
    @Override
    public LabelList operate(LabelList ll, LabelTest arg) {
  ...
  final LabeledLattice.LabelOp<Void> pendingLabelStripOp = new LabeledLattice.LabelOp<Void>() {
    @Override
  ...
  final LabeledLattice.Combiner<T, LoopMerge> loopMergeOp = new LabeledLattice.AbstractCombiner<T, LoopMerge>() {
    @Override
  ...
  public ForwardAnalysis(final String name, final L l, final XFER t, final IRNodeViewer nv, boolean timeOut) {
  ...
  protected void transferJoin(Join node) {
    ...
    if (node instanceof Merge) {
      ...
      doTransfer(in1, in2, out, addLabelOp, TrackLabel.trueTrack, addLabelOp, TrackLabel.falseTrack);
    } else {
  ...
  protected void transferSource(Source n) {
    ...
    setInfo(edge, new LabeledLattice.LabeledValue<>(LabelList.empty, val, null));
  }
  ...
  @Override
  @RequiresLock("ComputeLock")
  protected void transferPort(IOutputPort port, IInputPort dual) {
