 ...
 * This class was designed to avoid object construction.
 * Perhaps this is a ridiculous optimization, perhaps not.
 * @author boyland
 */
public final class LabeledLattice<T> {
  ...
  // a binary operator with an extra argument so we can avoid creating short-lived
  // instances.
  ...
  // a unary operator with an extra argument to avoid creating instances frequently.
  ...
  public static interface LabelOp<U> {
     ...
     * @param ll label list to operate on
     * @param arg extra argument necessary to do work and avoid mem-alloc
     * @return changed label list, or null if this label not applicable to operation
     */
  ...
  public static class LabeledValue<T2> {
    ...
    public LabeledValue(LabelList ll, T2 v, LabeledValue<T2> n) {
      if (v == null) throw new NullPointerException("Lattice values cannot be null");
      labelList = ll;
      value = v;
      next = n;
    }
  ...
  public T getValue(LabeledValue<T> lv, LabelList ll, T defaultValue) {
    while (lv != null) {
      if (lv.labelList == ll) return lv.value;
      lv = lv.next;
    }
    return defaultValue;
  }
  ...
  public LabeledValue<T> setValue(LabeledValue<T> lv, LabelList ll, T newValue) {
    ...
    if (value != secret) {
      if (lattice.equals(value,newValue)) return lv;
      lv = removeValue(lv,ll);
    }
    return new LabeledValue<T>(ll,newValue,lv);
  }
  
  protected LabeledValue<T> removeValue(LabeledValue<T> lv, LabelList ll) {
    if (lv.labelList == ll) return lv.next;
    return new LabeledValue<T>(lv.labelList,lv.value,removeValue(lv.next,ll));
  }
  
  public LabeledValue<T> join(LabeledValue<T> in1, LabeledValue<T> in2,
                                                         LabeledValue<T> out) {
   ...
   * from both sides have values.  This combiner is often but not always
   * join ({@link #joinCombiner}).  Since it isn't always join, the combiner
   * will sometimes be called with one argument bottom.
   * @param <U> Type of argument to pass to combiner
   * @param in1 first labeled value to merge
   * @param in2 second labeled value to merge
   * @param combiner combination function to run, no matter what.
   * @param arg argument to pass combiner
   * @param out previous result: reuse if possible.
   * @return
   */
  public <U> LabeledValue<T> merge(LabeledValue<T> in1, LabeledValue<T> in2, 
      Combiner<T,U> combiner, U arg, LabeledValue<T> out) {
    if (in1 == null) {
    ...
    T head;
    T v2 = getValue(in2,ll,secret); // get the value from the second labeled values
    ...
    if (out == null || out.labelList != ll && getValue(out,ll,secret) != secret) {
      return new LabeledValue<T>(ll,head,merge(lv1,lv2,combiner,arg,null));
    }
    LabeledValue<T> tail;
    if (out.labelList != ll) {
  ...
  public <U> LabeledValue<T> 
  map(LabeledValue<T> in, UnaryOp<T,U> op, U arg, LabeledValue<T> out) {
    ...
    if (out == null || out.labelList != ll && getValue(out,ll,secret) != secret) {
      return new LabeledValue<T>(ll,head,map(in.next,op,arg,null));
    }
    LabeledValue<T> tail;
    if (out.labelList != ll) {
  ...
  public <U> LabeledValue<T>
  labelMap(LabeledValue<T> in, LabelOp<U> op, U arg, LabeledValue<T> out) {
    if (in == null) return null;
    return labelMap2(in,op,arg,null,null,null,out);
    /*
    // for now, do the easiest, space inefficient way
    if (in ==null) return null;
    LabelList before = in.labelList;
    T value = in.value;
    LabelList after = op.operate(before,arg);
    if (after == null) {
      return labelMap(in.next,op,arg,out);
    }
    LabeledValue<T> tail;
    if (out == null || getValue(out,after,secret) == secret) {
      tail = labelMap(in.next,op,arg,out);
    } else if (out.labelList == after) {
      tail = labelMap(in.next,op,arg,out.next);
    } else {
      // space inefficient:
      tail = labelMap(in.next,op,arg,removeValue(out,after));
    }
    T newValue = value;
  ...
  public <U> LabeledValue<T>
  labelMap2(LabeledValue<T> in1, LabelOp<U> op1, U arg1, 
                       LabeledValue<T> in2, LabelOp<U> op2, U arg2, LabeledValue<T> out) {
    // TODO: improve efficiency if this causes lots of churning
    /*
     * so many different kinds of maps:
     * tracked merge/demerge: guaranteed to only filter, not merge values
     * LabelTest filter, drop and maybe merge
     * PendingLabelStrip drop and maybe merge.
     * How do we ensure that no memory is allocated in quiescent case? 
     */
    // for now, do the easiest, space inefficient way
    if (in1 ==null) return labelMap(in2,op2,arg2,out);
  ...
  protected LabeledValue<T> makeLabeledValue(LabelList ll, T head, LabeledValue<T> tail, LabeledValue<T> out) {
    ...
    return new LabeledValue<T>(ll,head,tail);
  }
  
  /**
   * Compare two labeled values and see if they are identical.
   * For simplicity, if the labels are in different order, the labeled
   * values are considered different.  If you want to compare lattice
   * equality, use equals
   * @param lv1
   * @param lv2
  ...
  public boolean identical(LabeledValue<T> lv1, LabeledValue<T> lv2) {
    while (lv1 != null && lv2 != null) {
      ...
      if (lv1.labelList != lv2.labelList || lv1.value != lv2.value) return false;
      lv1 = lv1.next;
      lv2 = lv2.next;
    }
    return lv1 == lv2;
  ...
  public T joinAll(LabeledValue<T> lv) {
    if (lv == null) return lattice.bottom();
    T res = lv.value;
  ...
  public String toString(LabeledValue<T> lv) {
    ...
    else return lv.toString(lattice);
  }
}
