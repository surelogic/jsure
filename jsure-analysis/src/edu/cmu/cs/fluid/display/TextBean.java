package edu.cmu.cs.fluid.display;
...
public class TextBean
  ...
  */
  ...
  public void paintComponent(final Graphics graphics) {
    ...
    final Iterator<StyleRange> styleEnum = styles.getStyles();
    StyleRange currentRange = null;
    {
      final TextCoord begin = new TextCoord(firstLine, 0);
      while (styleEnum.hasNext()) {
        currentRange = styleEnum.next();
    ...
    outer : while (true) {
      ...
      while (currentRange.getEnd().after(line, pos)) {
        ...
        if (!back.equals(defaultBack) || (numSecond != 0)) {
          final Iterator<Color> seconds = ts.getSecondaryBackground(defaultStyle);
          final int width =
    ...
    final Iterator<TextCoord> coords = garray.getTextCoords(firstLine, lastLine);
    while (coords.hasNext()) {
      final TextCoord tc = coords.next();
      ...
      final Iterator<InsertableGraphic> igs = garray.getGraphicsAt(tc);
      while (igs.hasNext()) {
        final InsertableGraphic ig = igs.next();
   ...
   * @param sy The y position to start drawing the background
  ...
  private void drawBackground(
    final Color primary,
    final int numSecond,
    final Iterator<Color> secondary,
    final Graphics g,
    final int sx,
    final int sy,
    final int width,
   ...
   * @param col The character position of the graphic
   * @param g The graphic to add
   */
   ...
   * @param font The font whose metrics to use
   */
  ...
  public TextCoord posUnderMouse(final Point p, final boolean flag) {
    ...
    if ((line >= 0) && (line < text.length)) {
      ...
      while (current == null) {
        final StyleRange sr = styleEnum.next();
        if (sr.inStyle(SOL)) {
          current = sr.getStyle();
          EOS = sr.getEnd();
        }
      ...
      while (ch < text[line].length() && !found) {
        ...
        if ((p.x < xoffset) && (p.x < vr.width)) {
          ...
          ch += 1;
          if (EOS.before(line, ch) && styleEnum.hasNext()) {
            final StyleRange sr = styleEnum.next();
            current = sr.getStyle();
            EOS = sr.getEnd();
            fm =
  ...
  private static final int autoScrollOffset = 5;
...
interface StyleManager {
  ...
  public abstract Iterator<StyleRange> getStyles();
...
final class StylesVector implements StyleManager, Serializable {
  private final Vector<StyleRange> list;
  @Override
  ...
  public TextStyle getStyleAt(final TextCoord where) {
    ...
    StyleRange sr = list.elementAt(styleNum);
...
class GraphicsArray implements Serializable {
  private TreeMap<TextCoord,Vector<InsertableGraphic>> graphicsMap;

  public GraphicsArray() {
    graphicsMap = new TreeMap<TextCoord,Vector<InsertableGraphic>>();
  }
  ...
  public void insert(final TextCoord tc, final InsertableGraphic g) {
    if (graphicsMap.containsKey(tc)) {
      final Vector<InsertableGraphic> v = graphicsMap.get(tc);
      v.addElement(g);
    } else {
      final Vector<InsertableGraphic> v = new Vector<InsertableGraphic>();
      v.addElement(g);
  ...
  public Iterator<TextCoord> getTextCoords(final int first, final int last) {
    final SortedMap<TextCoord,Vector<InsertableGraphic>> tail = graphicsMap.tailMap(new TextCoord(first, 0));
    final SortedMap<TextCoord,Vector<InsertableGraphic>> head = tail.headMap(new TextCoord(last + 1, 0));
    return head.keySet().iterator();
  }

  public Iterator<InsertableGraphic> getGraphicsAt(final TextCoord tc) {
    return graphicsMap.get(tc).iterator();
  }
}
