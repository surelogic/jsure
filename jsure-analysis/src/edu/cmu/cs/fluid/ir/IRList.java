...
/**
 * Variable size sequences with locations that stay valid under reshaping. For
 * efficiency, we have four different representations. The most general
 * representation is the doubly-linked list. This representation is explained in
 * the remainder of this description.
 * <p>
 * This implementation uses a free list. This allows us to assign fixed
 * IRLocation values that persist over list changes, without having a pool of
 * nodes that grows larger and larger with each list change. In order to have
 * this free list work correctly, the following invariants are needed:
 * <ol>
 * <li>The last element in the free list is always the element with the highest
 * ID. This permits us to attach new nodes to the end of the free list in
 * constant time.
 * <li>The free list does not use the "prev" link. It doesn't need to be
 * changed.
 * <li>The (next) links between elements in the free list are always valid: they
 * are initial values. This allows us to treat the free list as infinite in
 * length, it just hasn't all been revealed yet.
 * </ol>
 * We keep the first invariant by putting newly allocated elements on the end of
 * the list and reclaimed nodes on the front of the list. The list is never
 * permitted to go empty. The first invariant is handled by always allocated a
 * predefined slot for the "next" field of the last free element just before
 * attaching a newly allocated node to the end of the list.
 * </p>
 */
@Region("ListState")
...
public abstract class IRList<IntS, S, ES, T> extends IRAbstractSequence<S, T> {
  ...
  static final Logger LOG = SLLogger.getLogger("FLUID.ir");
  ...
  Header<IntS, S, ES, T> seq = EmptyHeader.prototype(); // dl-list of
                                                        // cells in
                                                        // sequence
  int initialSize; // starting size (often 0)
  IntS sizeSlot;
  ...
  /*
   * public SlotFactory getSlotFactory() { return slotFactory; }
   */
  ...
  private void initialize(int size, boolean reverse) {
    if (sizeSlot == null)
      initializeSizeSlot(size);
    SlotStorage<S, T> st = getSlotStorage();
    SlotList slots = new SlotList(st, size);
    /*
     * Moved to SlotList constructor for (int i=0; i < size; ++i) {
  ...
  private void checkInitialized(boolean reverse) {
    if (seq instanceof EmptyHeader)
      noLongerEmpty(reverse);
  }
  ...
  /**
   * Called before wendo an operation that requires that the header be fully
   * general
   */
  @RequiresLock("ListLock")
  void checkGeneral(String reason) {
    checkInitialized(false);
    if (seq instanceof IRList.FullHeader)
      return;
  ...
  public Iteratable<T> elements() {
    return getSlotFactory().newIterator(new IRSequenceIterator<>(this));
  }
  ...
  @RequiresLock("ListLock")
  IRLocation validateLocation(IRLocation loc) {
  ...
  public synchronized int compareLocations(IRLocation loc1, IRLocation loc2) {
    return seq.comparePlacements(validateLocation(loc1), validateLocation(loc2));
  }
  ...
  public static <T> IRSequence<T> readValue(IRInput in, IRSequence<T> current) throws IOException {
  ...
  @Override
  public synchronized void writeContents(IRCompoundType<IRSequence<T>> t, IROutput out) throws IOException {
    /*
     * Some of the values read/written here may be redundant, since we just
     * created the list with its initial size. But to avoid this problem would
     * be very tricky. We'd have to do something like "changed" contents for the
     * head,tail,free,prev&next fields but do the regular contents thing for the
     * elements. If/when we make this change, we will need to "up" the revision
     * number in IRPersistent, because it is not forward compatible.
     */
    if (out.debug())
      System.out.println("writeContents(...) called");
    // SlotFactory sf = getSlotFactory();
    if (sizeSlot == null) {
    ...
    IRType<T> et = (IRType<T>) t.getType(0); // must be homogenous
    // for persistent 1.5, we write a reference to the object
    seq.writeContents(et, out);
  }

  @Override
  public synchronized void readContents(IRCompoundType<IRSequence<T>> t, IRInput in) throws IOException {
    if (in.debug())
      System.out.println("readContents(...) called");
    @SuppressWarnings("unchecked")
    IRType<T> et = (IRType<T>) t.getType(0); // must be homogenous
    int b = 'F';
    // SlotFactory sf = getSlotFactory();
  ...
  public synchronized boolean isChanged() {
    ...
    if (ss.isChanged(sizeSlot) || seq.isChanged())
      return true;
    return false;
  }

  @Override
  public synchronized void writeChangedContents(IRCompoundType<IRSequence<T>> t, IROutput out) throws IOException {
    @SuppressWarnings("unchecked")
    IRType<T> et = (IRType<T>) t.getType(0); // must be homogenous
    // for persistent 1.5, we write a reference to the object
    ...
    // we read IRListLocation types. That is why we don't bother to write
    // ourselves.
    seq.writeChangedContents(et, out);
  }

  @Override
  public synchronized void readChangedContents(IRCompoundType<IRSequence<T>> t, IRInput in) throws IOException {
    @SuppressWarnings("unchecked")
    IRType<T> et = (IRType<T>) t.getType(0); // must be homogenous
    // SlotFactory sf = getSlotFactory();
    SlotStorage<IntS, Integer> ss = getIntSlotStorage();
  ...
  protected ListIteratable<T> createListIterator(int start) {
    return new IRSequence_ListIterator<>(this, start);
  }
  ...
  /**
   * The structure for the list nodes. All these methods assume that the list is
   * properly synchronizing all accesses.
   * 
   * @author boyland
   */
  ...
  static class EmptyHeader<IntS, S, ES, T> implements Header<IntS, S, ES, T> {
    ...
    @SuppressWarnings("unchecked")
    static final <IntS, S, ES, T> EmptyHeader<IntS, S, ES, T> prototype() {
      return (EmptyHeader<IntS, S, ES, T>) prototype;
    }
    ...
    @Override
    public void writeChangedContents(IRType<T> et, IROutput out) throws IOException {
  ...
  /**
   * A list in which we simple have a list of slots, one for each location. This
   * is permitted if nothing interesting happens to next/prev links.
   * 
   * @author boyland
   */
  @Promise("@Borrowed(this) for public *(**)")
  abstract class SimpleHeader extends CountInstances implements Header<IntS, S, ES, T> {
    ...
    protected S getElemSlot(int i) {
      ...
      /*
       * while (i >= elemSlots.size()) { elemSlots.add(emptySlot); }
       */
    ...
    public IRLocation validateLocation(IRLocation loc) {
      /*
       * Moved to SlotList.checkSize if (elemSlots.size() < size()) {
       * LOG.severe("BUG in IRList: elemSlots shorter than size!");
       * IRList.this.describe(System.err); }
       */
      return loc;
    ...
    public boolean isValidAt(IRLocation loc) {
      int i = loc.getID();
      if (i < 0 || i >= size() || i >= elemSlots.size())
        return false;
    ...
    protected void addElem(T elem) {
      ...
      S s = sf.newSlot();
      /*
       * We have to create the slot and then initialize in two steps to ensure
       * that the persistence system realizes that this slot is "changed". BUG
       * 892.
       */
  ...
  class AppendHeader extends SimpleHeader {
    ...
    public IRLocation next(IRLocation loc) {
      int i = loc.getID();
      if (++i >= size())
        return null; // may have gone back in time.
      return IRLocation.get(i);
    }
  ...
  /**
   * The header used in the full general case. It takes over from the simple
   * case in which things are added only at the end.
   * <p>
   * We assume that when this header is created, it is possible that the list
   * went through several lengthening extensions in which new locations were
   ...
   * hypothetical slot which remembers history). When asked for the "next" slot
   * or the last slot, we check against the sentinel first.
   * <p>
   * This class is also used as a fall-back for "insert headers" where the list
   * elements are each inserted in reverse order. In this case "head" and "prev"
   * links require the use of the sentinel. Happily, we never need to use the
   * sentinel in the same place in different ways.
   * <p>
   * Modifications make this information incorrect, because the nodes will no
   * longer be in order, and the current size doesn't tell us what comes next.
   * Thus whenever we have a modification when the current state has a sentinel
   * (we check "head" and "tail"), we convert the information to the full
   * information (without sentinels) for the new versions branching off here.
   * This involves changing a versioned slot, but since we're doing a
   * modification already, the delta is folded into the modification itself.
   * <p>
   * If we have something like a versioned this, this converting of sentinel
   * information into real information can happen over and over, an unbounded
   * number of times. (One may continually return to the last version that
   * originally used the simple header.) However, normally, it happens only
   * once.
   * <p>
  ...
  class FullHeader extends CountInstances implements Header<IntS, S, ES, T> {
    ...
    void alloc() {
      ...
      Element le = new Element(nextid++, getSlotStorage().newSlot());
      elements.add(le);
      /*
       * We initialize the element slot so that the persistence mechanism will
       * see that it is changed when this node is added to the list: otherwise
       * the initial value assigned will not show up as a change and be missed
       * as in Bug 892.
       */
      ...
      /*
       * le.next will need to be converted into a predefined slot before being
       * used.
       */
    }

    /**
     * Allocate the first element in the infinite sequence. Its prev field is
     * preset to null. Its next field is <em>not</em> set until later, until we
     * know what its <em>initial</em> value will be,
     * 
     * @see #newElement()
     */
    @Borrowed("this")
     ...
     * previous state. In particular, the size() has <em>never</em> been less
     * than the initial size and never greater than the number of slots
     * (maxSize).
     * 
     * @param maxSize
     *          the number of slots in the append header previously
     */
    @RequiresLock("IRList.this:ListLock")
    @Borrowed("this")
    protected void initForward(int maxSize) {
      ...
      if (maxSize == 0) {
        ...
        for (int i = 1; i < maxSize; ++i) {
          ...
          if (initialSize > i) { // next has never been anything other than to
                                 // next node
            p.next = ls.newSlot(n);
          } else {
            p.next = sentinelSlot;
    ...
    /**
     * Initialize the full header to handle the case that the slots were
     * originally in an {@link InsertHeader}.
     * 
     * @param maxSize
     *          size of slot list in insert header
     */
    @RequiresLock("IRList.this:ListLock")
    @Borrowed("this")
    protected void initBackward(int maxSize) {
      ...
      if (maxSize == 0) {
        ...
        for (int i = 1; i < maxSize; ++i) {
          ...
          if (initialSize > i) { // this node was brought into life with this
                                 // "prev"
            n.prev = ls.newSlot(p);
          } else {
            n.prev = sentinelSlot;
    ...
    /**
     * make sure we are ready to mutate slots. This can only we done when no
     * slots have sentinel state. Since if any have sentintel state, then the
     * head or tail will, we use this to check.
     */
    ...
    /**
     * Allocate a new sequence cell and overwrite the value in it to the
     * parameter value.
     */
    ...
    public void writeContents(IRType<T> et, IROutput out) throws IOException {
      ...
      if (out.debug())
        System.out.println("List has " + nextid + " elements");
      out.writeInt(nextid);
    ...
    public boolean isChanged() {
      SlotStorage<ES, IRLocation> ls = getElemStorage();
      if (ls.isChanged(head) || ls.isChanged(tail))
        return true;
      if (free != noSlot && ls.isChanged(free))
        return true;
      int nextid = elements == null ? 0 : elements.size();
      for (int i = 0; i < nextid; ++i) {
        Element le = elements.get(i);
    ...
    public void writeChangedContents(IRType<T> et, IROutput out) throws IOException {
      ...
      SlotStorage<ES, IRLocation> ls = getElemStorage();
      int flags = 0;
      if (ls.isChanged(head))
        flags |= 1;
      if (ls.isChanged(tail))
        flags |= 2;
      if (ls.isChanged(free))
        flags |= 4;
      out.writeByte(flags);
      IRType<IRLocation> locType = getLocationType();
      if ((flags & 1) != 0) {
    ...
    public void readChangedContents(IRType<T> et, IRInput in) throws IOException {
      ...
      int flags = in.readByte();
      if (in.debug()) {
        System.out.println("    head=" + ((flags & 1) != 0 ? "changed" : "unchanged") + " tail="
            + ((flags & 2) != 0 ? "changed" : "unchanged") + " free=" + ((flags & 4) != 0 ? "changed" : "unchanged"));
      }
      IRType<IRLocation> locType = getLocationType(in);
      if ((flags & 1) != 0) {
        head = ls.readSlotValue(head, locType, in);
      ...
      if ((in.getRevision() < 5 && in.readBoolean()) || (flags & 4) != 0) {
        if (in.debug())
          System.out.println("    free=changed");
        free = ls.readSlotValue(free, locType, in);
      } else {
        if (in.debug())
          System.out.println("    free=unchanged");
      }
      boolean allChanged = in.readBoolean();
      if (in.debug())
        System.out.println("    (" + (allChanged ? "all" : "not all") + " changed)");
      if (allChanged) {
        int n = in.readInt();
        for (int i = 0; i < n; ++i) {
          if (in.debug())
  ...
  /**
   * Generic Doubly-Linked list class to be used for IRList. The next slot may
   * be the sentinel location which means that the next location is the next
   * location ID unless we are past the end of the size(). This spcial case
   * permits us to start with the much more efficient structure SimpleHeader.
   * <p>
   * <bold>This class has race conditions in it concerning slots. Whenever we
   * have <blockquote> <tt>slot = slot.setValue(<i>e</i>) </blockquote>, this
   * needs to be protected somehow. </bold>
   */
  class Element extends IRLocation {
    ...
    Element getPrev() {
      ...
      if (loc == IRLocation.getSentinel()) {
        ...
        if (++i >= size())
          ...
          return (Element) seq.validateLocation(IRLocation.get(i));
      }
      return (Element) loc;
    }
    ...
    public void writeContents(IRType<T> et, IROutput out) throws IOException {
      ...
      SlotStorage<ES, IRLocation> ls = getElemStorage();
      SlotStorage<S, T> st = getSlotStorage();
      if (prev != noSlot && ls.isValid(prev))
        flags |= 1;
      if (next != noSlot && ls.isValid(next))
        flags |= 2;
      if (st.isValid(elem))
        flags |= ELEM_FLAG;
      out.writeByte(flags);
    ...
    public boolean isChanged() {
      SlotStorage<ES, IRLocation> ls = getElemStorage();
      return (prev != noSlot && ls.isChanged(prev)) || (next != noSlot && ls.isChanged(next)) || getSlotStorage().isChanged(elem);
    }
    ...
    public void writeChangedContents(IRType<T> et, IROutput out) throws IOException {
      ...
      SlotStorage<S, T> st = getSlotStorage();
      int flags = 0;
      if (prev != noSlot && ls.isChanged(prev))
        flags |= 1;
      if (next != noSlot && ls.isChanged(next))
        flags |= 2;
      if (st.isChanged(elem))
        flags |= ELEM_FLAG;
      out.writeByte(flags);
      if ((flags & 1) != 0) {
        ls.writeSlotValue(prev, getLocationType(), out);
    ...
    public void readChangedContents(IRType<T> et, IRInput in) throws IOException {
      ...
      int flags = in.readByte();
      if (in.debug()) {
        System.out.println("        prev=" + ((flags & 1) != 0 ? "changed" : "unchanged") + " next="
            + ((flags & 2) != 0 ? "changed" : "unchanged") + " elem=" + ((flags & ELEM_FLAG) != 0 ? "changed" : "unchanged"));
      }
      if ((flags & 1) != 0) {
        if (prev == noSlot)
          prev = ls.newSlot();
  ...
  static class IRListLocationType extends IRLocationType {
    ...
    public IRLocation readValue(IRInput in) throws IOException {
      ...
      if (in.debug()) {
        System.out.println("    location " + (loc == null ? "null" : Integer.toString(loc.getID())));
      }
      if (loc == null || loc == IRLocation.getSentinel())
        return loc;
      IRList<?, ?, ?, ?> l;
      if (in.getRevision() < 5) {
        l = list;
    ...
    public void writeValue(IRLocation l, IROutput out) throws IOException {
      if (out.debug()) {
        System.out.println("    location " + (l == null ? "null" : Integer.toString(l.getID())));
      }
      super.writeValue(l, out);
      if (l == null || l == IRLocation.getSentinel())
        return;
      ...
      if (l instanceof IRList.Element) {
        ...
        if (true)
          throw new IOException("Internal error: full list has an unvalidated location");
