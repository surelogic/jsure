...
public class TypeUtils {
  ...
  private final HashMap<Pair<IJavaReferenceType, IJavaReferenceType>, JavaRefTypeProxy> proxyCache = new HashMap<Pair<IJavaReferenceType, IJavaReferenceType>, JavaRefTypeProxy>();
  ...
  /**
   * Creates a proxy if there isn't one Gets the existing one otherwise
   */
   ...
   * bounds. Easier to swap the formals in this method than to fix the callsite
   * of this method.
   */
  ...
  // sec 15.12.2.7
  // Then, for each remaining type variable Tj, the constraints Tj :> U are
  // considered.
  // Given that these constraints are Tj :> U1 ... Tj :> Uk, the type of Tj is
  // inferred
  ...
  private Iterable<IJavaSourceRefType> getST(IJavaReferenceType... types) {
    ...
    for (IJavaReferenceType t : types) {
      ...
      getST(st, t);
      /*
       * for(IJavaType s : t.getSupertypes(tEnv)) { getST(st,
       * (IJavaDeclaredType) s); }
       */
    }
    return st;
  ...
  // The erased candidate set for type parameter Tj , EC, is the intersection of
  // all
  ...
  // For any element G of MEC that is a generic type declaration, define the
  // relevant
  ...
  // and let CandidateInvocation(G) = lci(Inv(G)) where lci, the least
  // containing
  ...
  private IJavaSourceRefType getLCI(Iterable<IJavaSourceRefType> invocations) {
    ...
    /*
     * TODO how does this ever help? if (num == 1) { return getLCP(result); }
     */
    return result;
  }
  ...
  private IJavaDeclaredType getLCI(IJavaDeclaredType t1, IJavaDeclaredType t2) {
    ...
    for (int i = 0; i < t1.getTypeParameters().size(); i++) {
      params.add(getLCTA(t1.getTypeParameters().get(i), t2.getTypeParameters().get(i)));
    }
  ...
  // lcta( U ) = ? if U 's upper bound is Object , otherwise ? extends lub( U ,
  // Object )
    ...
    if (u instanceof IJavaWildcardType) {
      if (v instanceof IJavaWildcardType) {
        ...
        if (v instanceof IJavaPrimitiveType) {
          // TODO is this right?
          //
          // This is a type bound, so most likely comes from something like
          // int.class
          return JavaTypeFactory.wildcardType;
        }
  ...
  // Then, define Candidate(W) = CandidateInvocation(W) if W is generic, W
  // otherwise.
  //
  // a.k.a. Best()
    ...
    if (bounds.length == 1) {
      ...
      for (IJavaReferenceType bt : ordered) {
        if (result == null) {
           ...
           * Nothing to do, since result subsumes bt } else if
           * (bt.isSubtype(binder.getTypeEnvironment(), result)) { // bt is more
           * specific than result result = bt;
           */
        } else {
  ...
  public class Constraints {
    ...
    /**
     * Constructor for a new set of constraints. The John-approved interface.
     * 
     * @param method
     *          method being called, should not be null
     * @param typeFormals,
     *          type formals of the method or of class for constructor
     * @param ikind
     *          kind of invocation context (must not be null)
     */
    ...
    public Constraints(CallState call, IBinding method, Map<IJavaType, IJavaType> substMap, boolean box, boolean varargs) {
    ...
    /**
     * Check that the actual parameter types work for formal parameter types. If
     * this is a var-arg method and allowVarArgs is true, then var-arg
     * invocation is possible. XXX: This method will need to be updated for Java
     * 8 if it is to be used for applicability checking. For Java 8, we don't
     * always have the parameter types.
     * 
     * @param formals
     *          list of formal types, must not be null
     * @param actuals
     *          list of actual types, must not be null
     * @param isVar
     *          the last formal is a var-arg formal
    ...
    public boolean deriveForParameters(List<? extends IJavaType> formals, List<? extends IJavaType> actuals, boolean isVar) {
      ...
      if (!allowVarargs || !isVar || asize < fsize - 1)
        return false;
    ...
    /**
     * 15.12.2.7 Inferring Type Arguments Based on Actual Arguments (really for
     * polymorphic methods)
     * 
    ...
    boolean derive(IJavaType formal, Constraint constraint, IJavaType actual) {
      ...
      if (formal instanceof IJavaPrimitiveType || actual instanceof IJavaNullType) {
        // Nothing to do since there can't be any type variables to deal with
        // here
        // or
      ...
      if (actual instanceof IJavaPrimitiveType) {
        ...
        // via boxing
        // conversion and this algorithm is applied recursively to the
        // constraint
        // U << F.
        ...
        // V[], or
        // a type variable with an upper bound that is an array type V[], where
        // V is a
        // reference type, this algorithm is applied recursively to the
        // constraint V<<U.
        ...
        if (MethodBinder.captureTypes2 || map.subst.containsKey(formal)) {
          ...
          // p.453: Otherwise, if F = Tj, then the constraint Tj :> A is
          // implied.
        ...
        return derive(it.getPrimarySupertype(), constraint, actual) | derive(it.getSecondarySupertype(), constraint, actual);
      } else if (formal instanceof IJavaWildcardType) {
        SLLogger.getLogger().info("Ignoring constraint: " + formal + " " + constraint + " " + actual);
        return false;
      } else
        throw new IllegalStateException("Unexpected type: " + formal);
    ...
    /**
     * p.453-4: Look for a supertype of a that matches f
     * 
     * @param c
     */
        ...
        if (f.getDeclaration().equals(a.getDeclaration())) {
          ...
          for (int i = 0; i < num; i++) {
            ...
            case CONVERTIBLE_FROM: // A << F
              derived |= deriveForTypeParameter(f.getTypeParameters().get(i), a.getTypeParameters().get(i));
              break;
            case EQUAL: // A = F
              derived |= deriveForTypeParameter_equal(f.getTypeParameters().get(i), a.getTypeParameters().get(i));
              break;
            /*
             * case CONVERTIBLE_TO: // A >> F derived |=
             * deriveForTypeParameter_to(f.getTypeParameters().get(i),
             * a.getTypeParameters().get(i)); break;
             */
            default:
              throw new UnsupportedOperationException();
            }
    ...
    private boolean deriveForTypeParameter(IJavaType fParam, IJavaType aParam) {
      if (fParam instanceof IJavaWildcardType) {
        // JLS 3 p.454:
        // If F has the form G<..., Yk-1, ? extends U, Yk+1, ...>, where U
        // involves Tj,
        ...
        if (u != null) {
          if (aParam instanceof IJavaWildcardType) {
            ...
            // G<..., Xk-1, V, Xk+1, ...>, where V is a type expression. Then
            // this algorithm
        ...
        // p.455:
        // If F has the form G<..., Yk-1, ? super U, Yk+1, ...>, where U
        // involves Tj,
        // then if A has a supertype that is one of:
          ...
          if (aParam instanceof IJavaWildcardType) {
            ...
            // G<..., Xk-1, V, Xk+1, ...>. Then this algorithm is applied
            // recursively to
            // the constraint V >> U.
        ...
        // expression that involves Tj, then if A has a supertype of the form
        // G<...,
        // Xk-1, V, Xk+1, ...> where V is a type expression, this algorithm is
        // applied
    ...
    private boolean deriveForTypeParameter_equal(IJavaType fParam, IJavaType aParam) {
      if (fParam instanceof IJavaWildcardType) {
        ...
        // Xk+1,...> where V is a type expression, this algorithm is applied
        // recursively
        // to the constraint V = U.
        return derive(fParam, Constraint.EQUAL, aParam);
      }
    ...
    private boolean deriveForDeclaredType_to(IJavaDeclaredType f, IJavaDeclaredType a) {
      if (a.getTypeParameters().isEmpty()) {
        // If A is an instance of a non-generic type, then no constraint is
        // implied on Tj.
        return false;
      }
      ...
      /*
       * IJavaDeclaredType h = findLowestCommonAncestor_raw(f, a); if (h !=
       * null) {
       */
    ...
    private boolean deriveForTypeParameter_to(IJavaDeclaredType f, IJavaType fParam, IJavaDeclaredType a, int i) {
      if (!f.getDeclaration().equals(a.getDeclaration())) {
        ...
        // let H<U1, ..., Ul> be the unique invocation of H that is a supertype
        // of G<S1, ..., Sn>
        // ...
        // make a version of F with just the type variables
        final IJavaDeclaredType pureF = (IJavaDeclaredType) tEnv.convertNodeTypeToIJavaType(f.getDeclaration());

        final Map<IJavaType, IJavaType> subst = Collections.singletonMap(pureF.getTypeParameters().get(i), fParam);
        ...
        if (fParam instanceof IJavaWildcardType) {
          ...
          if (fw.getUpperBound() != null) {
            ...
            // Then this algorithm is applied recursively to the constraint A >>
            // V.
          ...
          // Then, if V :> F this algorithm is applied recursively to the
          // constraint A >> V.
      ...
      if (fParam instanceof IJavaWildcardType) {
        ...
        if (aw != null) {
          ...
          // where W is a type expression, this algorithm is applied recursively
    ...
    private IJavaType transformParametersToWildcards(IJavaDeclaredType h, boolean useAsUpperBounds) {
      ...
      for (int i = 0; i < size; i++) {
        ...
        newParameters
            .add(useAsUpperBounds ? JavaTypeFactory.getWildcardType(null, old) : JavaTypeFactory.getWildcardType(old, null));
      }
    ...
    public Mapping computeTypeMapping() {
      if (useNewTypeInference) {
        ...
        // Make sure that the variables are at worst the same as the original
        // bound
    ...
    public IJavaTypeSubstitution getSubstitution() {
      if (map.subst.isEmpty())
        return IJavaTypeSubstitution.NULL;
      if (!map.checkIfSatisfiesBounds())
        return null;
  ...
  public class Mapping {
    ...
    Mapping(CallState call, IBinding method, Map<IJavaType, IJavaType> substMap) {
      this.call = call;
      this.method = method;
      subst = MethodBinder.captureTypes2 ? new IdentityHashMap<IJavaType, IJavaType>(substMap)
    ...
    /**
     * JLS v3 sec 15.12.2.5 Checks if Al <: Bl[R1 = A1, ..., Rp = Ap], extends
     */
  ...
  /**
   * For now, try to handle the simple cases 1. T appears as the type of a
   * parameter 2. T appears as the base type of an array parameter (T[]) 3. T
   * appears as a parameter of a parameterized type (Foo<T,T>) 4. T appears as a
   * bound on a wildcard (? extends T)
   * 
   * This no longer does any substitution
   */
  private void capture(Map<IJavaType, IJavaType> map, IJavaType fty, IJavaType argType) {
    if (fty == null) {
      return;
    }
    if (map.isEmpty()) {
    ...
    if (fty instanceof IJavaTypeFormal) {
      ...
      if (mapping instanceof IJavaTypeFormal/* == fty */) { // no substitution
                                                            // yet
  ...
  private void captureDeclaredType(final Map<IJavaType, IJavaType> map, final IJavaDeclaredType fdt, final IJavaDeclaredType adt) {
    final int size = fdt.getTypeParameters().size();
   ...
   */
  private void captureMissingTypeParameters(Map<IJavaType, IJavaType> map, final IJavaDeclaredType fdt,
  ...
  private IJavaType substitute(Map<IJavaType, IJavaType> map, IJavaType fty) {
    ...
    if (fty instanceof IJavaTypeFormal) {
      ...
      if (rv == fty) {
        ...
        /*
         * try { return substitute(map, rv); } catch(StackOverflowError e) {
         * System.err.println("Stack overflow on "+rv); throw new
      ...
      if (captured) {
        ...
        return JavaTypeFactory.getDeclaredType(dt.getDeclaration(), params, dt.getOuterType());
      }
    } else if (fty instanceof IJavaWildcardType) {
  ...
  private IJavaReferenceType substituteWildcardType(Map<IJavaType, IJavaType> map, IJavaType fty) {
  ...
  private final class TypeConstraint {
    ...
    public boolean equals(Object other) {
      if (other instanceof TypeConstraint) {
        ...
        return constraint == o.constraint && variable.equals(o.variable) && bound.equals(o.bound);
      }
      return false;
    }
  ...
  public enum Constraint {
    ...
    /**
     * U << V indicates that type U is convertible to type V by method
    ...
    /**
     * U >> V indicates that type V is convertible to type U by method
  ...
  private class GeneratedConstraints {
    ...
    final Multimap<IJavaTypeVariable, TypeConstraint> inequalities = ArrayListMultimap.create();
  }

  /**
   * JLS 3 p.463 -- If U is not one of the type parameters of the method, then U
   * is the type inferred for Tj. Then all remaining constraints involving Tj
   * are rewritten such that Tj is replaced with U. There are necessarily no
   * further equality constraints involving Tj, and processing continues with
   * the next type parameter, if any.
   * 
   * -- Otherwise, the constraint is of the form Tj = Tk for . Then all
   * constraints involving Tj are rewritten such that Tj is replaced with Tk,
   * and processing continues with the next type variable.
   * 
   * @return
   */
  GeneratedConstraints inferTypeParameters(Mapping map, Set<TypeConstraint> constraints) {
    /*
     * if (constraints.size() > 1) { System.out.println("Inferring from "
    ...
    for (TypeConstraint c : constraints) {
       ...
       * resolved as follows: For each implied equality constraint Tj = U or U =
       * Tj:
       */
      if (c.constraint == Constraint.EQUAL) {
        ...
        if (c.variable.equals(c.bound) || c.bound.equals(old)) {
          // Otherwise, if U is Tj, then this constraint carries no information
          // and may be
          // discarded.
          continue;
        } else if (old != null) {
          // Already checked above if the bound is the same as one processed
          // earlier
          // throw new IllegalStateException(c.variable+" already set to
          // "+old+", now to "+c.bound);
    ...
    // Do substitutions on the bounds using the equalities above, and organize
    // by variable
    for (TypeConstraint c : bounds) {
      ...
      if (c.bound.equals(newBound)) {
        ...
        generated.inequalities.put(c.variable, new TypeConstraint(c.variable, c.constraint, (IJavaReferenceType) newBound));
      }
    }
    /*
     * Then, for each remaining type variable Tj, the constraints Tj :> U are
     * considered. Given that these constraints are Tj :> U1 ... Tj :> Uk, the
     * type of Tj is inferred as lub(U1 ... Uk), computed as follows:
     */
    for (Map.Entry<IJavaTypeVariable, Collection<TypeConstraint>> e : generated.inequalities.asMap().entrySet()) {
      Set<IJavaReferenceType> temp = new HashSet<>(e.getValue().size());
      ...
      /*
       * if (inputs.length > 1) { System.out.println("Orig: "+e.getKey()+" => "
       * +map.subst.get(e.getKey())+", now "+lub); }
       */
   ...
   * 
   * If any of the method's type arguments were not inferred from the types of
   * the actual arguments, they are now inferred as follows. -- If the method
   ...
   * 
   * let R be the declared result type of the method, and let R' = R[T1=B(T1)
   * ... Tn=B(Tn)], where B(Ti) is the type inferred for Ti in the previous
   * section or Ti if no type was inferred.
   * 
   * Then, a set of initial constraints consisting of (see below)
   * 
   * is created and used to infer constraints on the type arguments using the
   * algorithm of �15.12.2.7.
   * 
   * Any equality constraints are resolved, and then, for each remaining
   * constraint of the form Ti <: Uk, the argument Ti is inferred to be glb(U1,
   * ..., Uk) (�5.1.10). If Ti appears as a type argument in any Uk, then Ti is
   * inferred to be a type variable X whose upper bound is the parameterized
   * type given by glb(U1[Ti=X], ..., Uk[Ti=X]) and whose lower bound is the
   * null type. Any remaining type variable T that has not yet been inferred is
   * then inferred to have type Object. If a previously inferred type variable P
   * uses T, then P is inferred to be P[T=Object].
   * 
   * -- Otherwise, the unresolved type arguments are inferred by invoking the
   * procedure described in this section under the assumption that the method
  ...
  Constraints handleUnresolvedVariables(final Mapping map, final GeneratedConstraints generated) {
    ...
    if (map.call.call != null) { // check in case we're just looking for
                                 // overridden parents
    ...
    for (Map.Entry<IJavaType, IJavaType> e : map.subst.entrySet()) {
      // -- additional constraints Bi[T1=B(T1) ... Tn=B(Tn)] >> Ti, where Bi is
      // the declared bound of Ti,
      // -- additional constraints B(Ti) << Bi[T1=B(T1) ... Tn=B(Tn)], where Bi
      // is the declared bound of Ti,
      final IJavaType t_i = e.getKey();
      final IJavaType b_i_subst = substitute(map.subst, t_i.getSuperclass(tEnv));
      constraints.derive(t_i, Constraint.CONVERTIBLE_TO, b_i_subst); // flipped
                                                                     // around
      constraints.derive(b_i_subst, Constraint.CONVERTIBLE_FROM, e.getValue());
    }
    // -- for any constraint of the form V >> Ti generated in �15.12.2.7: a
    // constraint V[T1=B(T1) ... Tn=B(Tn)] >> Ti.
    ...
    // -- for any constraint of the form Ti = V generated in �15.12.2.7: a
    // constraint Ti = V[T1=B(T1) ... Tn=B(Tn)].
    ...
    if (!stillUnresolved.isEmpty()) {
      // System.err.println(DebugUnparser.toString(map.call.call)+" not fully
      // resolved "+JavaNames.genRelativeFunctionName(map.method.getNode()));
      // SLLogger.getLogger().warning("Still has unresolved types:
      // "+stillUnresolved);
  ...
  /**
   * Check if the method result occurs in a context where it will be subject to
   * assignment conversion (�5.2) to a type S ...
   * 
   * If S is a reference type, then let S' be S. Otherwise, if S is a primitive
   * type, then let S' be the result of applying boxing conversion (�5.1.7) to
   * S.
   */
  ...
  private IJavaType computeReturnType(Mapping map) {
    ...
    // return rt.subst(FunctionParameterSubstitution.create(tEnv.getBinder(),
    // map.method.getNode(), map.subst));
  ...
  public IJavaType getPolyExpressionTargetType(final IRNode pe, boolean eliminateTypeVars) {
    ...
    if (AssignExpression.prototype.includes(op)) {
      ...
      /*
       * if (bi != null) { IRNode decl = bi.getNode(); if (decl != null) { final
       * IRNode formals, targs; Operator dop = JJNode.tree.getOperator(decl); if
       * (MethodDeclaration.prototype.includes(dop)) { formals =
       * MethodDeclaration.getParams(decl); targs =
       * MethodCall.getTypeArgs(call); } else if
       * (ConstructorDeclaration.prototype.includes(dop)) { formals =
       * ConstructorDeclaration.getParams(decl); // TODO what about
       * ConstructorCall/ACE? targs = NewExpression.getTypeArgs(call); } else {
       * //LOG.warning("what could a call be bound to? " + dop); return null; }
       * //return bi.convertType(tEnv.getBinder(),
       * tEnv.getBinder().getJavaType(JJNode.tree.getChild(formals, i)));
       * MethodBinder8 mb = new MethodBinder8((AbstractJavaBinder)
       * tEnv.getBinder(), false); CallState state = new
       * CallState(tEnv.getBinder(), call, targs, p, bi.getReceiverType());
       * IJavaFunctionType ftype = mb.computeInvocationType(state, bi); //return
       * ftype.getReturnType(); IJavaType ptype =
       * ftype.getParameterTypes().get(TypeUtil.isStatic(bi.getNode()) ? i :
       * i+1); return bi.convertType(tEnv.getBinder(), ptype); } }
       */
  ...
  /**
   * Find H, where A is the invocation of a generic type declaration H, where H
   * is either G or superclass or superinterface of G where G is the generic
   * type of F
   */
