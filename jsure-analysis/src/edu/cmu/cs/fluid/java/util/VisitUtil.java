...
public class VisitUtil implements JavaGlobals {
  ...
  // / When here is already a CU node, we just return here, rather than
  // searching above it.
   ...
   * 
   * <p>
   * <em>This is purely a syntactic operation.  There are times when it does
  ...
  public static final OpSearch varDeclsSearch = new OpSearch() {
    ...
    protected boolean found(Operator op) {
      return (op instanceof ParameterDeclaration) || (op instanceof DeclStatement) || (op instanceof FieldDeclaration);
    }
  };
      ...
      protected Object computeNext() {
        while (enm.hasNext()) {
          ...
          if ((op == MethodDeclaration.prototype)
              || (op == ConstructorDeclaration.prototype || (op == AnnotationElement.prototype))) {
            return n;
          }
        }
        return noElement;
    ...
    return new SimpleRemovelessIterator<IRNode>() {
      ...
      protected Object computeNext() {
        while (moreElements()) {
          ...
          if ((ClassDeclaration.prototype.includes(op)) || (InterfaceDeclaration.prototype.includes(op))
              || (AnonClassExpression.prototype.includes(op)) || EnumDeclaration.prototype.includes(op)
  ...
  public static IRNode computeOutermostEnclosingTypeOrCU(IRNode locInIR) {
    // figure out what the "cu" is as needed by ColorDeclareDrop,
    // TRoleRenameDrop
    // and ColorRenamePerCu should be. The rule is that we preferentially want
    // the
    ...
    if (ClassDeclaration.prototype.includes(op) || InterfaceDeclaration.prototype.includes(op)) {
      ...
      cu = locInIR;
    } else if (NestedClassDeclaration.prototype.includes(op) || NestedInterfaceDeclaration.prototype.includes(op)) {
