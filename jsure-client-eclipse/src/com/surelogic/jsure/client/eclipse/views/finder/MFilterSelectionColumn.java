...
public final class MFilterSelectionColumn extends MColumn implements IFilterObserver {
  ...
  void init() {
      ...
      public Composite createContents(Composite panel) {
        ...
        f_reportContents.addFocusListener(new FocusListener() {
          ...
          public void focusGained(FocusEvent e) {
            if (valueList == null || valueList.isEmpty()) {
              ...
              Color focusedText = f_reportContents.getDisplay().getSystemColor(SWT.COLOR_LIST_SELECTION_TEXT);
        ...
        f_reportContents.addKeyListener(new KeyListener() {
          ...
          private void handleChar(KeyEvent e) {
            /*
             * Handle space and return by ourselves (e.g. override default OS
             * behavior)
             */
        ...
        /**
         * Note that the next three listeners implement the bar graph See
        ...
        f_reportContents.addListener(SWT.MeasureItem, new Listener() {
          /**
           * The first custom draw event that is sent. This event gives a client
        ...
        f_reportContents.addListener(SWT.EraseItem, new Listener() {
          /**
           * Sent just before the background of a cell is about to be drawn. The
           * background consists of the cell's background color or the selection
           * background if the item is selected. This event allows a client to
           * custom draw one or both of these. Also, this event allows the
        ...
        f_reportContents.addListener(SWT.PaintItem, new Listener() {
          ...
          public void handleEvent(Event event) {
            if (event.index == 1) {
              ...
              final boolean mouseOverGraph = f_mouseOverLine == null ? value == null : f_mouseOverLine.equals(value);
              if (mouseOverGraph) {
        ...
        f_bottomSection.setLayoutData(new GridData(SWT.FILL, SWT.DEFAULT, true, false));
        ...
        f_searchBox = new SearchBox(f_bottomSection, "filter the list above", "Clear the current filter expression", observer);
        ...
        final GridData searchGridData = new GridData(SWT.FILL, SWT.CENTER, true, false);
        ...
        final GridData porousCountGridData = new GridData(SWT.FILL, SWT.CENTER, false, false);
    ...
    getCascadingList().addColumnAfter(c, getPreviousColumn().getColumnIndex(), false);
  ...
  private void updateReport() {
    ...
    f_totalCount.setText(SLUtility.toStringHumanWithCommas(total) + (total == 1 ? " Result" : " Results"));

    /*
     * Fix the value lines.
     */
    final List<String> valueList = f_sortByCount ? f_filter.getValuesOrderedBySummaryCount() : f_filter.getAllValues();
    ...
    if (currentRows != valueList.size()) {
      ...
      for (String row : valueList) {
        ...
        final boolean itemDiffers = row == null ? item.getData() != null : !row.equals(item.getData());
        if (itemDiffers) {
          updateData(item, row);
        }
        i++;
    ...
    if (f_porousCount != null && !f_porousCount.isDisposed()) {
      if (f_reportContents.getItemCount() > 0) {
        final String porousCountString = SLUtility.toStringHumanWithCommas(porousCount);
        f_porousCount.setText(porousCountString);
        String msg = (porousCount == 0 ? "No" : porousCountString) + (porousCount != 1 ? " results" : " result") + " selected";
