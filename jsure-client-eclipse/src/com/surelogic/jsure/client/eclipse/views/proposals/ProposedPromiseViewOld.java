...
public class ProposedPromiseViewOld extends AbstractScanStructuredView<IProposedPromiseDrop> implements
    EclipseUIUtility.IContextMenuFiller, IUninterestingPackageFilterObserver {

  private final ProposedPromiseViewContentProviderOld f_content;
  ...
  public ProposedPromiseViewOld() {
    super(SWT.MULTI, IProposedPromiseDrop.class);
    /*
     * Read persisted state of toggles that control what is displayed.
     */
    boolean persistedAsTree = EclipseUtility.getBooleanPreference(JSurePreferencesUtility.PROPOSED_PROMISES_AS_TREE);
     ...
     */
    f_toggleView = new Action(I18N.msg("jsure.eclipse.proposed.promises.showAsTree"), IAction.AS_CHECK_BOX) {
      @Override
      public void run() {
        setViewerBeingShown(f_toggleView.isChecked());
      }
    };
    f_toggleView.setImageDescriptor(SLImages.getImageDescriptor(CommonImages.IMG_JAVA_DECLS_TREE));
    f_toggleView.setToolTipText(I18N.msg("jsure.eclipse.proposed.promises.showAsTree.tip"));
    /*
     * Set the view to tree or table
     */
    f_toggleView.setChecked(persistedAsTree);
    f_content = new ProposedPromiseViewContentProviderOld(persistedAsTree);

    /*
     * Setup toggle to filter list of promises
     */
    f_toggleFilter = new Action(I18N.msg("jsure.eclipse.proposed.promises.showAbductiveOnly"), IAction.AS_CHECK_BOX) {
      @Override
      public void run() {
    ...
    f_toggleFilter.setToolTipText(I18N.msg("jsure.eclipse.proposed.promises.showAbductiveOnly.tip"));
    f_toggleFilter.setImageDescriptor(SLImages.getImageDescriptor(CommonImages.IMG_ANNOTATION_ABDUCTIVE));
    /*
  ...
  public void fillContextMenu(final IMenuManager manager, final IStructuredSelection s) {
    if (!s.isEmpty()) {
  ...
  protected StructuredViewer[] newViewers(Composite parent, int extraStyle) {
    ...
    final TreeViewer treeViewer = new TreeViewer(parent, SWT.H_SCROLL | SWT.V_SCROLL | extraStyle);
  ...
  private void setViewerBeingShown(final boolean asTree) {
     ...
     */
    EclipseUtility.setBooleanPreference(JSurePreferencesUtility.PROPOSED_PROMISES_AS_TREE, asTree);
    ...
    /*
     * We need a job to fix the toolbar/view menu because the viewer changed and
     * things may or may not be selected in the new view. This will do it a bit
     * later when the change settles.
     */
    Job job = new SLUIJob() {
      @Override
  ...
  private void setShowAbductiveOnly(final boolean applyFilter) {
     ...
     * Persist user preference
     */
    EclipseUtility.setBooleanPreference(JSurePreferencesUtility.PROPOSED_PROMISES_SHOW_ABDUCTIVE_ONLY, applyFilter);
  ...
  public void uninterestingPackageFilterChanged() {
    AbstractJSureScanView.notifyScanViewOfChangeIfOpened(ProposedPromiseViewOld.class);
  }
}
