...
public class ProposedPromisesChange {
	...
	void change(final CompositeChange root) {
		...
		final PromisesAnnotationRewriter rewrite = new PromisesAnnotationRewriter(
				PromiseFramework.getInstance().getAllowsMultipleAnnosSet());
		try {
			for (final String proj : projects) {
				...
				final IJavaProject selectedProject = JDTUtility
						.getJavaProject(proj);
				for (final IPackageFragment frag : selectedProject
						.getPackageFragments()) {
					for (final ICompilationUnit unit : frag
							.getCompilationUnits()) {
						final CU check = new CU(proj, frag.getElementName(),
								unit.getElementName());
	...
	static AnnotationDescription desc(final IProposedPromiseDropInfo drop) {
		...
		final CU cu = new CU(drop.getTargetProjectName(), srcRef.getPackage(),
				srcRef.getRelativePath());
		srcRef = drop.getAssumptionRef();
		final CU assumptionCU = new CU(drop.getFromProjectName(),
				srcRef.getPackage(), srcRef.getRelativePath());
		return new AnnotationDescription(annotation, contents,
				drop.getReplacedContents(), target, assumptionTarget, cu,
				assumptionCU);
	}
