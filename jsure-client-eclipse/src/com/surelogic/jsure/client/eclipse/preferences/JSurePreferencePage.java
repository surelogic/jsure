...
public class JSurePreferencePage extends AbstractCommonPreferencePage {
  ...
  protected Control createContents(Composite parent) {
    ...
    dataDirectory.setText(JSurePreferencesUtility.getJSureDataDirectory().getAbsolutePath());
    dataDirectory.setLayoutData(new GridData(SWT.FILL, SWT.CENTER, true, false));

    final Group xmlDiffDirGroup = createGroup(panel, "preference.page.group.xmlDiffDir");
    xmlDiffDirGroup.setLayout(new GridLayout(2, false));

    final Label xmlDiffDir = new Label(xmlDiffDirGroup, SWT.NONE);
    xmlDiffDir.setLayoutData(new GridData(SWT.FILL, SWT.CENTER, true, false));
    f_xmlDiffDir = new PathPreferenceEditor(xmlDiffDir, IDEPreferences.JSURE_XML_DIFF_DIRECTORY);
    f_xmlDiffDir.show();

    final Button change = new Button(xmlDiffDirGroup, SWT.PUSH);
    change.setText(I18N.msg("jsure.eclipse.preference.page.changexmlDiffDir"));
    change.setLayoutData(new GridData(SWT.DEFAULT, SWT.DEFAULT, false, false));
    change.addListener(SWT.Selection, new Listener() {
      @Override
      public void handleEvent(final Event event) {
        final File existing = JSurePreferencesUtility.getJSureXMLDirectory();
        final ChangeDirectoryLocationDialog dialog = new ChangeDirectoryLocationDialog(change.getShell(), existing, I18N
            .msg("jsure.eclipse.change.data.directory.dialog.title"), SLImages.getImage(CommonImages.IMG_JSURE_LOGO), I18N
            .msg("jsure.eclipse.change.data.directory.dialog.information"));
    ...
    f_balloonFlag = new BooleanFieldEditor(JSurePreferencesUtility.SHOW_BALLOON_NOTIFICATIONS,
    ...
    f_selectProjectsToScan = new BooleanFieldEditor(JSurePreferencesUtility.ALWAYS_ALLOW_USER_TO_SELECT_PROJECTS_TO_SCAN,
        I18N.msg("jsure.eclipse.preference.page.selectProjectsToScan"), diGroup);
    ...
    f_selectProjectsToUpdateJar = new BooleanFieldEditor(
        ...
        I18N.msg("jsure.eclipse.preference.page.selectProjectsToUpdateJar"), diGroup);
  ...
  private void setupThreadCount(Group group) {
    ...
    f_analysisThreadCount = new ScaleFieldEditor(IDEPreferences.ANALYSIS_THREAD_COUNT, I18N.msg(THREADS_LABEL, threads), group);
  ...
  private void setupTimeoutWarning(Group group) {
    int timeoutWarningSec = EclipseUtility.getIntPreference(IDEPreferences.TIMEOUT_WARNING_SEC);

    final String label = I18N.msg(TIMEOUT_WARNING_LABEL, timeoutWarningSec);
    f_timeoutWarningSec = new ScaleFieldEditor(IDEPreferences.TIMEOUT_WARNING_SEC, label + "     ", group);
    setupScaleEditor(group, f_timeoutWarningSec, 5, 600, 60, TIMEOUT_WARNING_LABEL);
  }

  private void setupTimeout(Group group) {
    int timeoutSec = EclipseUtility.getIntPreference(IDEPreferences.TIMEOUT_SEC);

    final String label = I18N.msg(TIMEOUT_LABEL, timeoutSec);
    f_timeoutSec = new ScaleFieldEditor(IDEPreferences.TIMEOUT_SEC, label + "     ", group);
