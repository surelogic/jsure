...
public final class LibraryAnnotationDialog extends TitleAreaDialog {
   ...
   * @param attributes
   *          a map of the annotation's attributes. This map is not modified by
   *          this call (any changes are returned in a new map).
   ...
   *         unempty map is returned it contains only keys with modified values
   *         from the values contained in the passed attributes map.
   * 
   * @throws IllegalArgumentException
   *           if either of the passed parameters are {@code null}.
   */
  public static Map<Attribute, String> edit(AnnotationElement annotation, Map<Attribute, String> attributes) {
    if (annotation == null)
      throw new IllegalArgumentException(I18N.err(33, "annotation"));
    if (attributes == null)
      throw new IllegalArgumentException(I18N.err(33, "attributes"));
    final LibraryAnnotationDialog dialog = new LibraryAnnotationDialog(annotation, attributes);
  ...
  public static void cannotEdit(String annotation) {
    final MessageDialog dialog = new MessageDialog(EclipseUIUtility.getShell(), I18N.msg("jsure.dialog.library.xml.title"),
        SLImages.getImage(CommonImages.IMG_ANNOTATION), I18N.msg("jsure.dialog.library.xml.noedit.msg", annotation),
   ...
   */
  final Map<Attribute, String> f_scratch;

  private LibraryAnnotationDialog(final AnnotationElement annotation, final Map<Attribute, String> attributes) {
    ...
    f_scratch = new HashMap<>(attributes);
  }
  ...
  protected Control createDialogArea(Composite parent) {
    ...
    if (!stringAttributes.isEmpty()) {
      ...
      final ScrolledComposite scroll = new ScrolledComposite(panel, SWT.V_SCROLL);
      ...
      for (final Attribute a : stringAttributes) {
        ...
        name.setForeground(getShell().getDisplay().getSystemColor(SWT.COLOR_BLUE));
        name.setLayoutData(new GridData(SWT.RIGHT, SWT.CENTER, false, false));
    ...
    updatePreviewAnnotationInDialog();
    setMessage(I18N.msg("jsure.dialog.library.xml.msg"), IMessageProvider.INFORMATION);
  ...
  /**
   * Filters {@link #f_attributes} down to a particular type and sorts the list
   * returned.
   * 
   * @param type
  ...
  private Map<Attribute, String> getModifiedAttributes() {
    final Map<Attribute, String> result = new HashMap<>();
  ...
  void updatePreviewAnnotationInDialog() {
