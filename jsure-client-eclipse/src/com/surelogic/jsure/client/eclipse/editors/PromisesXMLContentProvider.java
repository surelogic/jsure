...
public class PromisesXMLContentProvider extends AbstractContentProvider implements ITreeContentProvider, ITableLabelProvider {
  ...
  void save(final IProgressMonitor monitor) {
    ...
    try {
      ...
      /*
       * Maybe saving because the editor is "dirty" so we can't do the below
       * anymore if (!pkg.isDirty()) { // Try to delete any diffs if
      ...
      if (p != null) {
        /*
         * Don't create the writer until this block&mdash;doing it above creates
         * the file and holds it under Windows (causing a subtle bug with the
         * delete() call below if we don't write to the file).
         */
        final PromisesXMLWriter w = new PromisesXMLWriter(f);
        w.write(p);
      } else {
        /*
         * The file may not exist, but if it does delete it&mdash;we have no
         * diff to save.
         */
        if (f.exists()) {
          boolean result = f.delete();
          if (!result)
            SLLogger.getLogger().log(Level.SEVERE, I18N.err(246, f), new Exception());
        }
      }
      pkg.markAsClean();
