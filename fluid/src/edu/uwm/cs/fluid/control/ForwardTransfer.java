/*$Header: /cvs/fluid/fluid/src/edu/uwm/cs/fluid/control/ForwardTransfer.java,v 1.3 2006/04/19 02:59:15 boyland Exp $*/
...
public interface ForwardTransfer<T> {
   ...
   * @param node syntax node for source
   ...
   */
  public T transferComponentChoice(IRNode node, Object info,
      boolean flag, T before);

  /** Compute whether a label is a possible/impossible match.
   * For a true flag, return whether it is possible the label matches.
   * For a false flag, return whether it is possible the label doesn't match.
   * @param node the syntax for the component this node exists in.
   * @param info the extra information stored in the LabelTest node.
   ...
   */
  public boolean transferLabelTest(IRNode node, Object info,
      ControlLabel label, boolean flag);
  
  /**
   * Transfer flow information around to the top of a loop.
   * By default, this transfer function should just call the widening operator
   * for the lattice, but sometimes, we need to use the 
   * @param node loop node
   * @param initial lattice value from before loop
   * @param looped lattice value from bottom of loop.
