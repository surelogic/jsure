...
/**
 * Promise drop for "reads" and "writes" promises on methods established by the
 * uniqueness analysis.
 * 
...
public class MethodEffectsPromiseDrop extends PromiseDrop<EffectsSpecificationNode> {
  ...
  @Override
  public void setAAST(EffectsSpecificationNode s) {
    if (getAAST() == null) {
      super.setAAST(s);
    }
   ...
   * @return true if bad
   */
  protected final <T extends EffectsSpecificationNode> boolean checkAST(T s, T current) {
  ...
  public Iterable<EffectSpecificationNode> getDeclaredEffects() {
    if (reads != null) {
      if (writes != null) {
        return new AppendIterator<EffectSpecificationNode>(reads.getEffectList().iterator(), writes.getEffectList().iterator());
      } else {
        return getAAST().getEffectList();
      }
    }
