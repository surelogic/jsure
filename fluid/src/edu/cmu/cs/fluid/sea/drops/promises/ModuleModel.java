...
public final class ModuleModel extends ModelDrop {
  ...
  public String enclosedModuleNames;
  public final Set<IRNode> enclosedModuleIR;
  
  private boolean containsCode = false;

  public final IRNode module;
  ...
  private ModuleModel(String name) {
    ...
    enclosedModuleIR = new HashSet<IRNode>(0);
  ...
  private ModuleModel(String name, IRNode thePromise) {
    ...
    enclosedModuleIR = new HashSet<IRNode>();
    if (modIter.hasNext()) {
      do {
        IRNode mod = modIter.next();
        enclosedModuleIR.add(mod);
  ...
  public static void processWrapperModParents() {
    ...
    for (ModuleModel mod : safeValues) {
      for (IRNode mIR : mod.enclosedModuleIR) {
        ModuleModel encMD = query(Module.getId(mIR));
...
//          LOG.severe("BAD module enclosure: " +mod.name+ " should not contain " +encMD.name);
          ...
          for (ModuleDrop aModDecl : ModuleDrop.findWrappingDecl(mod.name, encMD.name)) {
  ...
  private boolean fixWrapperAPIs() {
    ...
    if (!origModuleAPI.isEmpty()) {
      ...
      for (IRNode where : origModuleAPI) {
         Set<ExportDrop> exps = ExportDrop.findExportDrop(where, module);
...
//    }
   ...
   */
  public boolean changeToWrapperMod(IRNode modulesIR) {
    if (moduleIsTheWorld()) {
      return false;
    }
      // all we need to do is add the new modules to the ones that are already
      // defined.
    Iterator<IRNode> modIter = Modules.getModuleIterator(modulesIR);
    StringBuilder sb = new StringBuilder();
