...
public class BinaryCUDrop extends CUDrop {
  ...
  static public CUDrop queryCU(String project, String javaName) {
    ...
    /*
     * @SuppressWarnings("unchecked") Set<BinaryCUDrop> drops =
     * Sea.getDefault().getDropsOfExactType(BinaryCUDrop.class); for
     * (BinaryCUDrop drop : drops) { ICodeFile file = drop.info.getFile();
     * String proj = file == null ? null : file.getProjectName(); if
     * (drop.javaOSFileName.equals(javaName) && proj == project) { return drop;
     * } }
     */
    return null;
  }
