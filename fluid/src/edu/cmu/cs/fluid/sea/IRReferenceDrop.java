...
public abstract class IRReferenceDrop extends Drop {
	...
	/**
	 * Sets the fAST node associated with this drop. The fAST node provided
	 * should be the {@link IRNode} that the analysis creating the result wishes
	 * to focus the user's attention on.
	 * 
	 * @param node
	 ...
	 * 
	 * @return null, if no such drop; the CUDrop if one is found; the first
	 *         CUDrop if more than one such drop is present. Note that the
	 *         returned CUDrop may be invalid.
	 */
	public final CUDrop getCUDeponent() {
		final Collection<? extends CUDrop> cus = Sea.filterDropsOfType(
				CUDrop.class, getDeponents());
	...
	public void addSupportingInformation(IRNode link, int num, Object... args) {
		if (num >= 0) {
			...
			ISupportingInformation info = new SupportingInformation2(link, num,
					args);
	 ...
	 * 
	 * @return the, possibly empty but non-null, set of proposed promises for
	 *         this drop.
	 */
