...
public class Report {
  private static String computeKey(Drop drop) {
    ...
    if (drop instanceof ResultDrop) {
      key = (key + (((ResultDrop) drop).isConsistent() ? "_PLUS" : "_REDX"))
          .intern();
    }
    return key;
  }
...
//    Set<Drop> drops = Sea.getDefault().getDropsOfType(Drop.class);
  ...
  public static Set<Drop> getDropsAssociatedWithModel(ModelDrop model) {
    /*
    if (model.hasDeponents()) {
      // Not an independent model (probably a RegionModel)
    ...
    */
    ...
    final DropPredicate pred = new AbstractDropPredicate() {
      ...
      public boolean match(Drop d) {
        if (drops.contains(d) || d instanceof ModelDrop) {
          // the model drop will be handled separately
          return false;
        }
        d.addMatchingDependentsTo(drops, this);
    ...
    model.addMatchingDependentsTo(drops, pred);  
    return drops;
  }
  ...
  public static Set<Drop> getAssociatedDrops(final Set<Drop> drops, final Drop startDrop) {  
    ...
    // adds recursively with the predicate
    startDrop.addMatchingDependentsTo(drops, pred);  
    return drops;
  }
  ...
  public static void generateReport(PrintWriter out) {
    ...
    DateFormat currentFormat = DateFormat.getDateTimeInstance(
...
//      ResultDrop result = i.next();
      ...
      out.println((result.isConsistent() ? "\"+\"  " : "\"X\"  ")
          + result.getMessage());
      ISrcRef sr = result.getSrcRef();
      if (sr != null) {
        Object file = sr.getEnclosingFile();
        if (file != null) {
          out.println("          in " + sr.getEnclosingFile()
              + " [line " + sr.getLineNumber() + "]");
        }
      }
      ...
      for (PromiseDrop promise : result.getChecks()) {
        if (first) {
          first = false;
          out
          .println("     Promises this result CHECKS (partially establishes):");
        }
      ...
      for (PromiseDrop promise : result.getTrusts()) {
        if (first) {
          first = false;
          out
          .println("     Promises this result TRUSTS (requires as preconditions):");
        }
        out.println("     | " + promise.getMessage());
        sr = promise.getSrcRef();
        if (sr != null) {
