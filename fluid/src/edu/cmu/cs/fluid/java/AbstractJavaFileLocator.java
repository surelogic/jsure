...
public abstract class AbstractJavaFileLocator<T,P> implements IJavaFileLocator<T,P> {
  ...
  protected static <T,P> void finishInit(AbstractJavaFileLocator<T,P> l) {
	  if (useIRPaging) {
		  IDE.getInstance().getMemoryPolicy().addLowMemoryHandler(
				  l.memHandler);
  ...
  public Iterator<IJavaFileStatus<T>> iterator() {
	  return new FilterIterator<Map.Entry<T,JavaFileStatus<T,P>>, IJavaFileStatus<T>>(
  ...
  protected long commitCurrentArchive(boolean force) throws IOException {
	  ...
	  if (LOG.isLoggable(Level.FINE))
		  LOG.fine("Committed " + numWritten + " resources ("
  ...
  private void persistAll(boolean force) throws IOException {
	  ...
	  if (LOG.isLoggable(Level.FINE)) {
		  if (num != 0) {
			  LOG.fine("Persisted " + num + " resources (" + length
					  + " bytes) in " + ((end - start) / 1000000) + " ms");
		  } else {
			  LOG.fine("Persisted " + length + " bytes in "
  ...
  private JavaFileStatus<T,P> getStatusForRegion(IRRegion owner,
		  boolean failOnError) {
	  if (owner == null) {
		  return null;
	  }
	   ...
	   */
	  if (failOnError) {
		  throw new IllegalArgumentException("Couldn't find status for "
				  + owner);
	  }
	  return null;
  }
...
public IJavaFileStatus<T> isUpToDate(T id, long time, Type thisType) {
  ...
  protected class MemoryHandler implements ILowMemoryHandler {
	  ...
	  public void handleLowMemory(IMemoryPolicy mp) {
		  ...
		  if (numUnloaded > 0) {
			  if (LOG.isLoggable(Level.FINE))
				  LOG.fine("Randomly unloaded ... " + numUnloaded
						  + " out of " + numEligible);
			  SlotInfo.gc();
  ...
  public synchronized void testReload() throws IOException {
	  ...
	  long end = System.nanoTime();
	  if (LOG.isLoggable(Level.FINE))
		  LOG.fine("Reloaded " + num + " resources in "
				  + ((end - start) / 1000000) + " ms");
  }
  ...
  public synchronized void testUnload(boolean test, boolean expectExceptions) {
		  ...
		  if (e.getValue().unload()) {
			  if (test) {
				  try {
					  ...
					  assert expectExceptions : "No exception on "
						  + e.getKey();
				  } catch (SlotUndefinedException sue) {
					  assert !expectExceptions : "Unexpected SlotUndefinedEx on "
						  + e.getKey();
				  }
			  }
			  num++;
	  ...
	  long end = System.nanoTime();
	  if (LOG.isLoggable(Level.FINE))
		  LOG.fine("Unloaded " + num + " resources in "
				  + ((end - start) / 1000000) + " ms");
  }
}
