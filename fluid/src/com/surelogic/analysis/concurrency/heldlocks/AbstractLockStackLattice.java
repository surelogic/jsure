 ...
 * ArrayLattice, where the Locks are mapped to array locations; this
 * mapping is referenced using the {@link #getIndexOf} method.  The domain of the 
 * lattice is {@code ImmutableList<ImmutableSet<IRNode>>[]}.
 *
 * @author aarong
 */
abstract class AbstractLockStackLattice extends 
  ...
  protected static HeldLock[] constructLockArray(
    ...
    /* (2) Add locks due to single threadedness.  Locks cannot be duplicates
     * of locks from (1) because constructors cannot requires locks on the 
     * receiver.
     */
  ...
  /**
   * Push the given method call onto the stack.  The affected locks are 
   * derived from the receiver of the method call.
   * 
   * @param oldValue
   * @param call
   * @param binder
  ...
  /**
   * Remove the top method call based on the given lock method call.
   * The affected locks are derived from the receiver of the method call.
   * 
   * @param oldValue
   * @param call
   * @param binder
   ...
   * 
   * @return The set of calls at the top of stack for the given lock,
   *         or {@code null} if the top value is the poisoned TOP
   *         set value.
   */
