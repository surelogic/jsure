...
public class SourceAdapter extends AbstractAdapter implements TreeVisitor<IRNode,CodeContext> {
	...
	/**
	 * Look for a comment at the beginning of a block
	 * Either /** or //@
	 */
	...
	private ISrcRef addSrcRef(Tree t, IRNode result) {		
		...
		if (comment != null && AnnotationVisitor.allowJavadoc(jp.getTypeEnv())) {
			JavaNode.setComment(result, comment);
			ref = new JavadocSourceRef(cuRef, start, end, line, comment);
		} else {
	...
	protected abstract class AbstractSourceFunction<T extends Tree> 
	...
	private IRNode adaptType(Tree t, CodeContext context) {
		...
		case IDENTIFIER:
			...
			return NamedType.createNode(id.getName().toString());
			*/
		case MEMBER_SELECT: // TypeRef
			/*
			MemberSelectTree mst = (MemberSelectTree) t;
			IRNode base = adaptType(mst.getExpression());
			return TypeRef.createNode(base, mst.getIdentifier().toString());
			*/
	...
	private class AdaptMembers extends AbstractSourceFunction<Tree> {
		...
		public IRNode callFunc(Tree t, CodeContext context) {
			...
			case BLOCK:				
				...
				return ClassInitializer.createNode(isStatic ? JavaNode.STATIC : 0,
						                           block);
			case METHOD:
    ...
    */
    ...
    public IRNode visitArrayType(ArrayTypeTree node, CodeContext context)
    {   // FIX inefficient    	
    ...
    private IRNode adaptClass(ClassTree node, CodeContext context, boolean isNested) {
    	...
    	final TypeKind kind = isInterface ? TypeKind.IFACE :
    		                  isEnum ? TypeKind.ENUM : TypeKind.OTHER;    	  
    	if (isAnno) {
        ...
        */
        ...
        if (isInterface) {        	
        	if (isNested) {
        		rv = NestedInterfaceDeclaration.createNode(annos, mods,
                        id, formals, Extensions.createNode(impl), body);         
        	} else {
        	    rv = InterfaceDeclaration.createNode(annos, mods,
        			                               id, formals, Extensions.createNode(impl), body); 
        	}
            addSrcRef(node, rv);
        ...
        if (isEnum) {
        	if (isNested) {
        		mods |= JavaNode.STATIC;   
        		rv = NestedEnumDeclaration.createNode(annos, mods,
                                                        id, impls, body);
        	} else {
        	    rv = EnumDeclaration.createNode(annos, mods,
        			                          id, impls, body);
        	}
            addSrcRef(node, rv);
        ...
        if (isNested) {
            rv = NestedClassDeclaration.createNode(annos, mods,
                    id, formals, ext, impls, body);
        } else {
    ...
    public IRNode visitCompilationUnit(CompilationUnitTree node, CodeContext context)
    {
        ...
        if (node.getPackageName() == null) {
        	...
        	pkg = NamedPackageDeclaration.createNode(Annotations.createNode(annos), 
        			                                 pkgName);
        	addSrcRef(node, pkg);
        }
        ...
        return CompilationUnit.createNode(pkg, ImportDeclarations.createNode(imports), 
    ...
    public IRNode visitForLoop(ForLoopTree node, CodeContext context)
    {
        ...
        if (node.getInitializer().isEmpty()) {
        	...
        	if (decls.length == 1) {
        		...
        		// Different types, so we need to create a separate block to accommodate that
    ...
    /*
    // Squashes the whole name into one SimpleName
    private IRNode adaptImportType(Tree t) {
    	
    	IRNode n = null;
    	switch (t.getKind()) {
    	case MEMBER_SELECT:
    		MemberSelectTree mst = (MemberSelectTree) t;
    		IRNode base          = SimpleName.createNode(mst.getExpression().toString());
    		n = QualifiedName.createNode(base, mst.getIdentifier().toString());
    		break;
    	case IDENTIFIER:
    		n = SimpleName.createNode(t.toString());
    		break;
    	}
    	return NameType.createNode(n);
    }
    */
    ...
    {   
    	...
    	try {
    		...
    		/*
    		if (unparse.equals("com.acclamation.config.xml") || unparse.equals("com.acclamation.datawrappers")) {
    ...
    public IRNode adaptMethod(MethodTree node, String className, TypeKind kind, CodeContext context) {
        ...
        IRNode[] fmls = map(varargs ? adaptVarargsParameters : adaptParameters, 
        		            node.getParameters(), context);
        ...
        if (rType == null) {        	
        	...
        	IRNode rv = ConstructorDeclaration.createNode(annos, mods,
        			                                 TypeFormals.createNode(typs), className, 
        			                                 Parameters.createNode(fmls), 
        ...
        IRNode rv = MethodDeclaration.createNode(annos, mods,
        		                            TypeFormals.createNode(typs), rType, id, 
        		                            Parameters.createNode(fmls), 0, 
        		                            Throws.createNode(exs), body);
    ...
    public IRNode visitMethodInvocation(MethodInvocationTree node, CodeContext context)
    {
        ...
        if (node.getMethodSelect() instanceof MemberSelectTree) {
        	...
        	if (inner != null) {
        		IRNode call = NonPolymorphicConstructorCall.createNode(inner, 
        				                                               Arguments.createNode(args));
        ...
        if (targs.length > 0) {
        	return PolymorphicMethodCall.createNode(object, TypeActuals.createNode(targs), 
        			                                method, Arguments.createNode(args));
        }
    ...
    public IRNode visitNewArray(NewArrayTree node, CodeContext context)
    {
        ...
        if (base == null) {
    		...
    		if (ElementValueArrayInitializer.prototype.includes(op)
    				|| edu.cmu.cs.fluid.java.operator.Annotation.prototype
    						.includes(op)) {
        ...
        if (init == null) {
            return ArrayCreationExpression.createNode(base, DimExprs.createNode(dims), unalloc,
                                                      NoArrayInitializer.prototype.jjtCreate());
        } else {
            return ArrayCreationExpression.createNode(base, DimExprs.createNode(dims), unalloc,
                                                      ArrayInitializer.createNode(init));
        }
    }
    ...
    public IRNode visitNewClass(NewClassTree node, CodeContext context)
    {
        ...
        if (tArgs.length == 0) {
            ...
            result = PolymorphicNewExpression.createNode(TypeActuals.createNode(tArgs), id, 
        		                                         Arguments.createNode(args));     
        }
	...
	private IRNode makeAnonClassBody(NewClassTree node, CodeContext context) {
		IRNode[] mbrs = map(new AdaptMembers(null, TypeKind.OTHER), 
				            node.getClassBody().getMembers(), context);
