...
public class DropDiff extends DiffNode implements IViewable {
  ...
  static DropDiff compute(String title, PrintStream out, String label, DiffNode n, DiffNode o) {
    ...
    // }
    /*
     * final Map<String, DiffNode> oldDetails = extractDetails(o.drop); final
     * Map<String, DiffNode> newDetails = extractDetails(n.drop); final
     * List<String> temp = new ArrayList<String>(); // Remove matching ones for
     * (String ns : newDetails.keySet()) { DiffNode oe = oldDetails.remove(ns);
     * if (oe != null) { temp.add(ns); } } for (String match : temp) {
     * newDetails.remove(match); }
     * 
     * if (oldDetails.isEmpty() && newDetails.isEmpty()) { DiffMessage m =
     * diffProperties(title, out, label, n, o); if (m != null) { return new
     * DropDiff(n.drop, o.drop, wrap(m)); } else { return null; } } if (title !=
     * null) { out.println(title); } out.println("\tDiffs in details for: " +
     * DiffCategory.toString(n)); for (String old : sort(oldDetails.keySet(),
     * temp)) { out.println("\t\tOld    : " + old); DiffNode e =
     * oldDetails.get(old); e.setAsOld(); } for (String newMsg :
     * sort(newDetails.keySet(), temp)) { out.println("\t\tNewer  : " + newMsg);
     * DiffNode e = newDetails.get(newMsg); e.setAsNewer(); }
     * List<AbstractDiffNode> remaining = new ArrayList<AbstractDiffNode>(1 +
     * oldDetails.size() + newDetails.size()); DiffMessage m =
     * diffProperties(null, out, label, n, o); if (m != null) {
     * remaining.add(m); } remaining.addAll(oldDetails.values());
     * remaining.addAll(newDetails.values()); Collections.sort(remaining);
  ...
  private static final ProofPredicate[] predicates = { new ProofPredicate("provedConsistent") {
    @Override
