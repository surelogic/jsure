...
public class CustomizableHashCodeMap<K, V> extends AbstractHashMap<K, V> {
  ...
  public V get(Object key) {
    ...
    // case 1
    if (hash(entry.key.hashCode()) == hashCode && isEqualKey(key, entry.key)) {
  ...
  public boolean containsKey(Object key) {
    ...
    HashEntry<K, V> entry = data[hashIndex(hashCode, data.length)];
    while (entry != null) {
      if (hash(entry.key.hashCode()) == hashCode && isEqualKey(key, entry.getKey())) {
        return true;
      }
      entry = entry.next;
    }
        ...
        if (entry != null) {
          ...
          do {
            ...
            if (isValidEntry(entry)) {
              int index = hashIndex(hash(entry.key.hashCode()), newCapacity);
  ...
  /**
   * Gets the values view of the map. Changes made to the view affect this map.
  ...
  protected Object clone() {
    try {
      CustomizableHashCodeMap<K, V> cloned = (CustomizableHashCodeMap<K, V>) super.clone();
  ...
  public static void main(String[] args) {
    ...
    /*
     * IRNode foo = new MarkedIRNode("foo"); map.put(foo, "bar");
     * System.out.println(map.get(foo));
     * 
     * String[] names = { "a", "b", "c", "d", "e", "f", "g" }; IRNode[] nodes =
     * new IRNode[names.length]; for (int i = 0; i < nodes.length; i++) {
     * nodes[i] = new MarkedIRNode(names[i]); map.put(nodes[i], nodes[i]); } for
     * (IRNode n : nodes) { System.out.println(map.get(n)); }
     */
