...
abstract class AbstractJavaElement implements IJavaElement {
  ...
  public PackageElement getRootParent() {
    ...
    if (e instanceof PackageElement) {
      ...
      if (e != null)
        ...
        throw new IllegalStateException("No package at the root of XML tree: " + this);
    }
  }
   ...
   */
  protected <T extends IMergeableElement> boolean mergeList(IJavaElement parent, List<T> orig, List<T> other, MergeType type) {
    ...
    final Set<T> deleted = type == MergeType.JSURE_TO_LOCAL ? new HashSet<T>() : Collections.<T> emptySet();
  ...
  protected <T extends IMergeableElement> List<T> handleNonconflictingChanges(List<T> orig, List<T> other, MergeType type) {
    ...
    final List<Delta> nonConflicts = new ArrayList<>();
    deltas: for (final Delta d : p.getDeltas()) {
      ...
      if (d instanceof InsertDelta) {
        ...
        checkAndFailIf(type == MergeType.LOCAL_TO_JSURE, "Deletes should be explicitly marked for " + type);
    ...
    for (Delta delta : nonConflicts)
      filtered.addDelta(delta);
    try {
   ...
   */
  protected static <T extends IMergeableElement> void filterDeleted(Collection<? super T> result, Collection<T> orig) {
    for (T e : orig) {
      if (e.isToBeDeleted()) {
        continue;
      }
