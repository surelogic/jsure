...
public class PackageElement extends AnnotatedJavaElement {
  ...
  public PackageElement(boolean confirmed, String id, int releaseVersion, ClassElement c) {
  ...
  /**
   * Performs a deep merge of this with the passed tree, this tree is mutated if
   * the merge is successful. The trees must be of the same package or type or
   * the merge is not performed and an exception is thrown.
   * 
   * @param other
  ...
  void mergeDeep(PackageElement other, MergeType typeOfMerge) {
    if (!other.getName().equals(getName()))
      throw new IllegalArgumentException(I18N.err(240, getName(), other.getName()));
    ...
    if (clazz == null && other.clazz == null) {
      ...
      final MergeResult<ClassElement> r = clazz.merge(other.clazz, typeOfMerge);
      if (r.element != null) {
        // Class merged, so continue merging
        mergeThis(other, typeOfMerge);
        return; // done
      } else
        throw new IllegalArgumentException(I18N.err(242, getName(), other.getName()));
    } else
      throw new IllegalArgumentException(I18N.err(241, getName(), other.getName()));
  }
  ...
  public PackageElement cloneMe(IJavaElement parent) {
    ...
    PackageElement e = new PackageElement(isConfirmed(), getName(), f_releaseVersion, cl);
    copyToClone(e);
    return e;
  }
  ...
  PackageElement copyIfDirty() {
    if (isDirty()) {
      ...
      PackageElement p = new PackageElement(isConfirmed(), getName(), f_releaseVersion, c);
      copyIfDirty(p);
      return p;
    }
    return null;
