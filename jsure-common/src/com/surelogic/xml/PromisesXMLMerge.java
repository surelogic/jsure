...
public class PromisesXMLMerge implements TestXMLParserConstants {
   ...
   * <p>
   * <i>Warning: This method only works if the tool user is running from source
   * code in a meta-Eclipse.</i>
   * 
   * @param local
  ...
  public static void mergeLocalXMLIntoJSureXML(File local, File jsure) {
    SLLogger.getLogger().log(Level.INFO, "mergeLocalXMLIntoJSureXML(local promises -> " + local + ", jsure promises -> " + jsure);
    ...
    try {
      ...
      if (localPE == null) {
        SLLogger.getLogger().log(Level.SEVERE, I18N.err(244, local), new Exception());
        return;
      }
      ...
      /*
       * We need to handle the startup case where the file may not exist in
       * JSure yet.
       */
      if (jsure.exists()) {
        ...
        if (jsurePE == null) {
          SLLogger.getLogger().log(Level.SEVERE, I18N.err(244, jsure), new Exception());
          return;
        }
      ...
      SLLogger.getLogger().log(Level.SEVERE, I18N.err(237, local, jsure), e);
    }
  }

  /**
   * Rewrites a JSure release promises XML file optionally incrementing the
   * release version. This method may be used to update the file format after a
  ...
  public static void rewriteJSureXML(File jsure, boolean incrementVersion) {
    SLLogger.getLogger().log(Level.INFO, "rewriteJSureXML(jsure promises -> " + jsure);
  ...
  public static void mergeJSureXMLIntoLocalXML(File local, File jsure) {
    ...
    boolean precondition = jsure.isFile() && jsure.exists() && local.isFile() && local.exists();
    if (!precondition)
      return; // nothing to do
    ...
    try {
      ...
      /*
       * We only need to process the local file if the release file has a higher
       * version number.
       */
      final int jsureRelVer = jsurePE.getReleaseVersion();
      final int localRelVer = localPE.getReleaseVersion();
      if (jsureRelVer > localRelVer) {
        SLLogger.getLogger().log(Level.INFO, "mergeJSureXMLIntoLocalXML(local promises -> " + local + "(version=" + localRelVer
            + "), jsure promises -> " + jsure + "(version=" + jsureRelVer + ")");
      ...
      SLLogger.getLogger().log(Level.SEVERE, I18N.err(238, jsure, local), e);
    }
  }
  ...
  /**
   * Removes cached attributes used for diffing&mdash;these are not meant to be
   * persisted.
   */
  ...
  /**
   * Marks the annotations that are changed as dirty, so we can figure out what
   * to keep when we output a diff file.
   */
  ...
  private static class Cleaner extends AbstractJavaElementVisitor<Void> {
    ...
    public Void visit(ClassElement c) {
      super.visit(c);
      /*
       * These checks work because we've removed any unannotated declarations
       * above.
       */
