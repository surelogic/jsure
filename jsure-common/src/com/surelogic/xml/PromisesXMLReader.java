 ...
 * @author Edwin
 */
public class PromisesXMLReader extends NestedJSureXmlReader implements IXmlResultListener, TestXMLParserConstants {
  private PackageElement pkg;
  private String pkgName;
  private int releaseVersion = 0;
  private ClassElement clazz;
  private final List<AnnotationElement> promises = new ArrayList<>(0);
  ...
  protected String checkForRoot(String name, Attributes attributes) {
    ...
    if (PACKAGE.equals(name)) {
      ...
      if (rev == null) {
        /*
         * Try the old "revision" attribute (for backwards compatibility).
         * 
         * Output a warning about this file it is in an obsolete format.
         */
        rev = attributes.getValue("revision");
        LOG.warning(pkg + " is still using 'revision' attribute not '" + RELEASE_VERSION_ATTRB + "'");
      }
      if (rev != null) {
        try {
          releaseVersion = Integer.parseInt(rev);
        } catch (NumberFormatException e) {
          LOG.warning("Bad release version for package " + pkg + ": " + rev + " (defaulting to 0)");
        }
      }
      return pkg;
    }
  ...
  private void handleNestedElement(ClassElement c, Entity n) {
    ...
    if (METHOD.equals(name)) {
      ...
      c.addMember(handleNestedElements(new NestedClassElement(false, id, Access.PUBLIC), n));
    } else if (CONSTRUCTOR.equals(name)) {
      c.addMember(handleNestedElements(new ConstructorElement(n), n));
    } else if (FIELD.endsWith(name)) {
      c.addMember(handleAnnotations(new FieldElement(false, id, Access.PUBLIC), n));
  ...
  private static void handleAnnotationOnElt(AnnotatedJavaElement func, Entity n) {
  ...
  private static final Map<String, PackageElement> cache = new WeakHashMap<>();
