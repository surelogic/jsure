...
public class PackageAccessor implements TestXMLParserConstants {
  ...
  public static String packageExtension(File f) {
    ...
    /**
     * Promise files are expected to be in the format: classname.promise.xml or
     * classname.promise.zx for zipped xml
     */
    if (fname.length < 3)
      return null;
  ...
  public static OutputStreamWriter writePackage(String pkgName, String className) throws IOException {
  ...
  public static InputSource readPackage(String pkgName, String className) throws IOException {
    ...
    final InputStream is = new FileInputStream(new File(root, dirName + className));

    return new InputSource(is);
  }

  public static OutputStreamWriter writeZipPackage(String pkgName, String className) throws IOException {
    String dirName = packagePath(pkgName);
  ...
  public static InputSource readZipPackage(String pkgName, String className) throws IOException {
    String dirName = packagePath(pkgName);
  ...
  public static Iterable<String> findPromiseXMLs() {
    try {
      final String localPath = IDERoot.getInstance().getStringPreference(IDEPreferences.JSURE_XML_DIFF_DIRECTORY);
      ...
      if (root.exists() && root.isDirectory()) {
        ...
        XmlCollector qnames = findPromiseXMLsInDir(new File(root, FileUtility.JSURE_XML_DIFF_PATH_FRAGMENT), localExists);
        if (localExists) {
