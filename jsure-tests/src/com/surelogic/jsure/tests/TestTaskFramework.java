...
public class TestTaskFramework extends TestCase {
  ...
  public void testSingleThreadedAsynchronousRun() {
    System.out.println("--------------testSingleThreadedAsynchronousRun----------------");
    TaskManager mgr = new TaskManager(1, 1, 60, TimeUnit.SECONDS, new LinkedBlockingQueue<Runnable>());
  ...
  public void testThreadThrowing() {
    ...
    TaskManager mgr = new TaskManager(1, 1, 60, TimeUnit.SECONDS, new LinkedBlockingQueue<Runnable>(), true);
    createGraphNoCycles(mgr);
  ...
  public void testSingleThreadedRunTimeout() {
    System.out.println("--------------testSingleThreadedRunTimeout---------------");
    TaskManager mgr = new TaskManager(1, 1, 60, TimeUnit.SECONDS, new LinkedBlockingQueue<Runnable>());
    createGraphNoCycles(mgr);
  ...
  public void testMultithreadedRun() {
    ...
    TaskManager mgr = new TaskManager(2, 4, 60, TimeUnit.MILLISECONDS, new LinkedBlockingQueue<Runnable>());
    createGraphNoCycles(mgr);
    ...
    mgr.addListener(new TaskListenerAdapter() {
      ...
      public void allTasksComplete() {
        ...
        assertTrue((e2.equals(TWO) && e3.equals(THREE)) || (e2.equals(THREE) && e3.equals(TWO)));

        String e4 = o.elementAt(3);
        String e5 = o.elementAt(4);

        assertTrue((e4.equals(FOUR) && e5.equals(FIVE)) || (e4.equals(FIVE) && e5.equals(FOUR)));
        assertEquals(o.elementAt(5), SIX);
        latch.countDown();
      }
    });
  ...
  public void testMultithreadedRun2() {
    ...
    TaskManager mgr = new TaskManager(4, 8, 60, TimeUnit.MILLISECONDS, new LinkedBlockingQueue<Runnable>());
  ...
  public void testNameClashes() {
    ...
    TaskManager tm = new TaskManager(1, 1, 60, TimeUnit.SECONDS, new LinkedBlockingQueue<Runnable>());
  ...
  public void testUndefinedDependencies() {
    System.out.println("--------------testUndefinedDependencies----------------");
    TaskManager tm = new TaskManager(1, 1, 60, TimeUnit.SECONDS, new LinkedBlockingQueue<Runnable>());
    createGraphNoCycles(tm);
  ...
  public void testOutOfOrderDependencyDeclarations() {
    System.out.println("--------------testOutOfOrderDependency----------------");
    TaskManager tm = new TaskManager(1, 1, 60, TimeUnit.SECONDS, new LinkedBlockingQueue<Runnable>());
  ...
  private static class TaskListener implements ITaskListener {
    ...
    public void allTasksComplete() {
      ...
      assertTrue(allCompletedCalled && allStartingCalled && completedCalled && taskStartedCalled);
      latch.countDown();
    }
