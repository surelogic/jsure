...
public class TestSea extends TestCase {
	...
	public void testFiterDrops() {
		...
		assertTrue("Drop count is " + r1.size() + " should be 6",
				r1.size() == 6);

		Set<MyDrop> r2 = Sea.filterDropsOfType(MyDrop.class, r);
		assertTrue("Drop count is " + r2.size() + " should be 4",
				r2.size() == 4);

		Set<MySubDrop> r3 = Sea.filterDropsOfType(MySubDrop.class, r);
		assertTrue("Drop count is " + r3.size() + " should be 2",
				r3.size() == 2);
		...
		assertTrue("Drop count is " + r1.size() + " should be 0",
				r1.size() == 0);

		r2 = Sea.filterDropsOfExactType(MyDrop.class, r);
		assertTrue("Drop count is " + r2.size() + " should be 2",
				r2.size() == 2);
	...
	public void testFilterDropsMutable() {
		...
		Set<MyDrop> r2 = Sea.filterDropsOfTypeMutate(MyDrop.class, r);
		assertTrue("Drop count is " + r2.size() + " should be 4",
				r2.size() == 4);
		assertEquals(r2, r);
		...
		Set<MySubDrop> r3 = Sea.filterDropsOfTypeMutate(MySubDrop.class, r);
		assertTrue("Drop count is " + r3.size() + " should be 2",
				r3.size() == 2);
		assertEquals(r3, r);
	...
	public void testFilter() {
		...
		Set<Drop> r1 = Sea.filter(DropPredicateFactory.matchType(ADrop.class),
				r);
		assertTrue("Drop count is " + r1.size() + " should be 6",
				r1.size() == 6);
		r1 = Sea.filter(DropPredicateFactory.matchType(MyDrop.class), r);
		assertTrue("Drop count is " + r1.size() + " should be 4",
				r1.size() == 4);
	...
	public void testHasMatchingDrops() {
		Set<MyDrop> r2 = Sea.filterDropsOfExactTypeMutate(MyDrop.class, Sea
				.getDefault().getDrops());
		assertTrue("Drop count is " + r2.size() + " should be 2",
				r2.size() == 2);
		assertFalse(Sea.hasMatchingDrops(DropPredicateFactory
				.matchExactType(Drop.class), r2));
		assertFalse(Sea.hasMatchingDrops(DropPredicateFactory
				.matchExactType(ADrop.class), r2));
		assertTrue(Sea.hasMatchingDrops(DropPredicateFactory
				.matchExactType(MyDrop.class), r2));
		assertFalse(Sea.hasMatchingDrops(DropPredicateFactory
				.matchExactType(MySubDrop.class), r2));
	}

	public void testAddMatchingDrops() {
		Set<Drop> r = sea.getDrops();
		Set<Drop> r1 = new HashSet<Drop>();
		Sea.addMatchingDropsFrom(r, DropPredicateFactory
				.matchExactType(ADrop.class), r1);
	...
	public void testGetDropsOfType() {
		...
		// drops
		assertTrue("Drop count is " + dropSet.size() + " should be 6", dropSet
				.size() == 6);
		...
		assertTrue("MyDrop count is " + myDropSet.size() + " should be 4",
				myDropSet.size() == 4);
		n = new HashSet<Drop>();
		n.add(md1);
		n.add(md2);
		...
		assertTrue(
				"MySubDrop count is " + mySubDropSet.size() + " should be 2",
				mySubDropSet.size() == 2);
		n = new HashSet<Drop>();
		n.add(msd1);
		n.add(msd2);
	...
	public void testGetDropsOfExactType() {
		...
		assertTrue("Drop count is " + dropSet.size() + " should be 2", dropSet
				.size() == 2);
		Set<Drop> n = new HashSet<Drop>();
		n.add(d1);
		n.add(d2);
		...
		assertTrue("MyDrop count is " + myDropSet.size() + " should be 2",
				myDropSet.size() == 2);
		n = new HashSet<Drop>();
		n.add(md1);
		n.add(md2);
