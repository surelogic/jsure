...
public class TestColumnAnnotationParser extends TestCase {
  ...
  public void testCount() {
    ...
    assertNull(c.getCountReplaceValues());
    assertFalse(c.countDistinct());
    assertEquals(Collections.emptySet(), c.getOnSet());
    assertEquals("", c.getSuffix());
  ...
  public void testCountReplaceValuesWithOne() {
    c = ColumnAnnotationParserUtility.parse("(count replace-values-with '1')");
    assertTrue(c.isValid());
    assertTrue(c.countPartialRows());
    assertEquals(c.getCountReplaceValues(), "1");
    assertFalse(c.countDistinct());
    assertEquals(Collections.emptySet(), c.getOnSet());
    assertEquals("", c.getSuffix());
    c = ColumnAnnotationParserUtility.parse("(count replace-values-with 'one' ' ms')");
    assertTrue(c.isValid());
    assertTrue(c.countPartialRows());
    assertEquals(c.getCountReplaceValues(), "one");
    assertFalse(c.countDistinct());
    assertEquals(Collections.emptySet(), c.getOnSet());
    assertEquals(" ms", c.getSuffix());
    c = ColumnAnnotationParserUtility.parse("(count replace-values-with 'one' on 1 ' ms')");
    assertTrue(c.isValid());
    assertTrue(c.countPartialRows());
    assertEquals(c.getCountReplaceValues(), "one");
    assertFalse(c.countDistinct());
    assertEquals(one, c.getOnSet());
    assertEquals(" ms", c.getSuffix());
    c = ColumnAnnotationParserUtility.parse("(count replace-values-with '1' on 1,2,3,4,5 ' ms')");
    assertTrue(c.isValid());
    assertTrue(c.countPartialRows());
    assertEquals(c.getCountReplaceValues(), "1");
    assertFalse(c.countDistinct());
  ...
  public void testCountReplaceValuesWithOneDistinct() {
    c = ColumnAnnotationParserUtility.parse("(count distinct replace-values-with 'one')");
    assertTrue(c.isValid());
    assertTrue(c.countPartialRows());
    assertEquals(c.getCountReplaceValues(), "one");
    assertTrue(c.countDistinct());
    assertEquals(Collections.emptySet(), c.getOnSet());
    assertEquals("", c.getSuffix());
    c = ColumnAnnotationParserUtility.parse("(count distinct replace-values-with 'one' ' ms')");
    assertTrue(c.isValid());
    assertTrue(c.countPartialRows());
    assertEquals(c.getCountReplaceValues(), "one");
    assertTrue(c.countDistinct());
    assertEquals(Collections.emptySet(), c.getOnSet());
    assertEquals(" ms", c.getSuffix());
    c = ColumnAnnotationParserUtility.parse("(count distinct replace-values-with 'one' on 1 ' ms')");
    assertTrue(c.isValid());
    assertTrue(c.countPartialRows());
    assertEquals(c.getCountReplaceValues(), "one");
    assertTrue(c.countDistinct());
    assertEquals(one, c.getOnSet());
    assertEquals(" ms", c.getSuffix());
    c = ColumnAnnotationParserUtility.parse("(count distinct replace-values-with 'one' on 1,2,3,4,5 ' ms')");
    assertTrue(c.isValid());
    assertTrue(c.countPartialRows());
    assertEquals(c.getCountReplaceValues(), "one");
    assertTrue(c.countDistinct());
